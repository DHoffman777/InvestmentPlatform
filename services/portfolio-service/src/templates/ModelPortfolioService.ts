...
export interface ModelPortfolioTemplate {
  ...
  rebalancingThreshold: number; // Percentage deviation to trigger rebalancing
...
export interface AllocationRanges {
  ...
  fixedIncome: { min: number; max: number; target: number };
  alternatives: { min: number; max: number; target: number };
  cash: { min: number; max: number; target: number };
  commodities?: { min: number; max: number; target: number };
  realEstate?: { min: number; max: number; target: number };
  international?: { min: number; max: number; target: number };
}
...
export interface FeeStructure {
  ...
  performanceFee?: number; // Percentage of excess returns
  transactionFee?: number; // Per transaction
  minimumFee?: number; // Annual minimum
  feeWaivers?: FeeWaiver[];
}

export interface FeeWaiver {
  condition: string;
  waiver: number; // Percentage or fixed amount
  expiry?: Date;
}
...
export interface ModelHolding {
  ...
  rebalancingPriority: number; // 1-10, higher = higher priority
...
export interface PerformanceTargets {
  ...
  sharpeRatio?: number; // Target Sharpe ratio
...
export interface PortfolioReplication {
  ...
  cashBuffer: number; // Percentage to keep as cash
 ...
 */
export class ModelPortfolioService extends EventEmitter {
  private templates: Map<string, ModelPortfolioTemplate> = new Map();
  private replications: Map<string, PortfolioReplication> = new Map();
   ...
   */
  public async createTemplate(template: Omit<ModelPortfolioTemplate, 'id' | 'createdAt' | 'updatedAt' | 'version'>): Promise<ModelPortfolioTemplate> {
    try {
      ...
      const newTemplate: ModelPortfolioTemplate = {
        ...template,
        id: templateId,
        createdAt: now,
        updatedAt: now,
   ...
   */
  public async updateTemplate(templateId: string, updates: Partial<ModelPortfolioTemplate>): Promise<ModelPortfolioTemplate> {
    try {
      ...
      const updatedTemplate: ModelPortfolioTemplate = {
        ...existingTemplate,
        ...updates,
   ...
   */
  public async replicateTemplate(
    templateId: string,
    targetPortfolioId: string,
    replicationConfig: Partial<PortfolioReplication>
  ): Promise<PortfolioReplication> {
    try {
      ...
      const replication: PortfolioReplication = {
   ...
   * Get template by ID
   */
  public getTemplate(templateId: string): ModelPortfolioTemplate | null {
  ...
  public async cloneTemplate(
    ...
    createdBy: string
  ): Promise<ModelPortfolioTemplate> {
    try {
      ...
      delete (clonedTemplate as any).version;
  ...
  private validateAllocation(allocation: AssetAllocation): void {
  ...
  private validateHoldings(holdings: ModelHolding[]): void {
  ...
  private mergeHoldings(original: ModelHolding[], modifications: Partial<ModelHolding>[]): ModelHolding[] {
  ...
  private async getEstimatedPrice(securityId: string): Promise<number> {
    // In production, this would fetch from market data service
    return 100; // Placeholder
  }

  private calculateTradingCosts(trades: RebalancingTrade[]): number {
  ...
  private initializeDefaultTemplates(): void {
    ...
    const conservativeTemplate: Omit<ModelPortfolioTemplate, 'id' | 'createdAt' | 'updatedAt' | 'version'> = {
