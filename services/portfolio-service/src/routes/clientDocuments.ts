...
const clientDocumentUploadSchema = [
  ...
  body('documentType').optional().isIn(Object.values(DocumentType)).withMessage('Invalid document type'),
  body('accessLevel').optional().isIn(Object.values(AccessLevel)).withMessage('Invalid access level'),
...
const documentUpdateSchema = [
  ...
  body('tags').optional().isArray().withMessage('Tags must be an array'),
  body('accessLevel').optional().isIn(Object.values(AccessLevel)).withMessage('Invalid access level'),
  body('status').optional().isIn(Object.values(DocumentStatus)).withMessage('Invalid status')
];
...
const documentShareSchema = [
  ...
  body('permissions.*').isIn(Object.values(DocumentPermission)).withMessage('Invalid permission'),
...
router.get('/client/:clientId',
  ...
  [
    ...
    query('status').optional().isIn(Object.values(DocumentStatus)).withMessage('Invalid status'),
  ...
  async (req: any, res: any) => {
    try {
      ...
      logger.info('Retrieving client documents', {
        tenantId,
        clientId,
        options,
        userId
      ...
      const result = await clientDocumentService.getClientDocuments(
        tenantId,
        clientId,
        userId,
        options
...
router.get('/:documentId',
  ...
  async (req: any, res: any) => {
    try {
      ...
      const document = await clientDocumentService.getClientDocumentById(
        tenantId,
        documentId,
        userId
      );
...
router.put('/:documentId',
  ...
  async (req: any, res: any) => {
    try {
      ...
      const updatedDocument = await clientDocumentService.updateClientDocument(
        tenantId,
        documentId,
        req.body,
        userId
...
router.delete('/:documentId',
  ...
  async (req: any, res: any) => {
    try {
      ...
      await clientDocumentService.deleteClientDocument(
        tenantId,
        documentId,
        userId,
        reason
...
router.post('/:documentId/share',
  ...
  async (req: any, res: any) => {
    try {
      ...
      await clientDocumentService.shareDocumentWithClient(
        tenantId,
        documentId,
        recipientClientId,
        permissions,
...
router.get('/templates',
  ...
  async (req: any, res: any) => {
    try {
      ...
      logger.info('Retrieving document templates', {
        tenantId,
        documentType
      });

      const templates = await clientDocumentService.getClientDocumentTemplates(
        tenantId,
        documentType
      );
...
router.post('/bulk-operation',
  ...
  async (req: any, res: any) => {
    try {
      ...
      const result = await clientDocumentService.performBulkDocumentOperation(
        tenantId,
        operation,
        userId
      );
...
router.get('/client/:clientId/stats',
  ...
  async (req: any, res: any) => {
    try {
      ...
      logger.info('Retrieving client document statistics', {
        tenantId,
        clientId
      });

      const stats = await clientDocumentService.getClientDocumentStats(
        tenantId,
        clientId
      );
