...
const assetClassificationService = new AssetClassificationService(prisma, kafkaService);
...
router.get('/asset-classes', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (liquidityTier) filters.liquidityTier = liquidityTier as string;
    if (parentClassId) filters.parentClassId = parentClassId as string;
...
router.get('/asset-subclasses', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const subClasses = await assetClassificationService.getAssetSubClasses(
      req.user!.tenantId,
      assetClassId as string
    );
...
router.get('/instruments/:instrumentId/classification', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const classification = await assetClassificationService.getInstrumentClassification(
      instrumentId,
      req.user!.tenantId
    );
...
router.get('/allocations', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const allocations = await assetClassificationService.getAssetAllocations(
      req.user!.tenantId,
...
router.post('/allocations', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const totalPercentage = allocations.reduce((sum: number, alloc: any) => sum + (alloc.targetPercentage || 0), 0);
...
router.get('/portfolios/:portfolioId/analysis', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const analysis = await assetClassificationService.analyzePortfolioClassification(
      portfolioId,
      req.user!.tenantId
    );
