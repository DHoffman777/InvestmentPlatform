...
router.get('/positions/:positionId', authenticateToken as any, async (req: any, res: any) => {
  try {
    ...
    try {
      ...
      logger.warn('Could not calculate yield metrics for position:', yieldError);
      yieldMetrics = null;
    }
    ...
    try {
      ...
      logger.warn('Could not valuate position:', valuationError);
      valuation = null;
    }
...
router.get('/portfolios/:portfolioId/fixed-income/summary', authenticateToken as any, async (req: any, res: any) => {
  try {
    ...
    positions.forEach(pos => {
      ...
      if (yearsToMaturity < 1) maturityBuckets['Under 1 Year'] += pos.marketValue;
      else if (yearsToMaturity < 3) maturityBuckets['1-3 Years'] += pos.marketValue;
      else if (yearsToMaturity < 5) maturityBuckets['3-5 Years'] += pos.marketValue;
      else if (yearsToMaturity < 10) maturityBuckets['5-10 Years'] += pos.marketValue;
      else maturityBuckets['Over 10 Years'] += pos.marketValue;
    });
