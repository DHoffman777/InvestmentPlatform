...
export class FixedIncomeService {
  ...
  async getFixedIncomePositions(portfolioId: string, tenantId: string): Promise<FixedIncomePosition[]> {
    try {
  ...
  async createFixedIncomePosition(request: CreateFixedIncomePositionRequest): Promise<FixedIncomePosition> {
    try {
      ...
      const accruedInterest = this.calculateAccruedInterest(
        request.faceValue * request.quantity,
        request.couponRate || 0,
        new Date(),
        new Date(request.maturityDate)
      );
  ...
  async createFixedIncomeTransaction(request: FixedIncomeTradeRequest): Promise<FixedIncomeTransaction> {
    try {
      const faceValue = 1000; // Standard bond face value
      const netAmount = request.quantity * request.price * faceValue / 100;
      const accruedInterest = this.calculateAccruedInterest(
        faceValue * request.quantity,
        0, // Will be updated with actual coupon rate
        new Date(request.tradeDate),
  ...
  async processCouponPayment(request: CouponProcessingRequest): Promise<CouponPayment> {
    try {
  ...
  async calculateYieldMetrics(positionId: string, tenantId: string): Promise<YieldCalculationResult> {
    try {
      const position = await this.prisma.position.findFirst({
        where: {
          id: positionId,
  ...
  async valuatePosition(request: FixedIncomeValuationRequest): Promise<any> {
    try {
      const position = await this.prisma.position.findFirst({
        where: {
          id: request.positionId,
      ...
      const valuation = {
        ...
        accruedInterest: this.calculateAccruedInterest(
          position.quantity.toNumber() * faceValue,
          metadata?.couponRate || 0,
          new Date(),
          new Date(metadata?.maturityDate || Date.now() + 365 * 24 * 60 * 60 * 1000)
        ),
  ...
  private calculateAccruedInterest(faceValue: number, couponRate: number, settlementDate: Date, lastCouponDate: Date): number {
  ...
  private approximateYTM(price: number, faceValue: number, couponRate: number, yearsToMaturity: number): number {
  ...
  private calculateDuration(price: number, faceValue: number, couponRate: number, yearsToMaturity: number): number {
  ...
  private calculateConvexity(price: number, faceValue: number, couponRate: number, yearsToMaturity: number): number {
