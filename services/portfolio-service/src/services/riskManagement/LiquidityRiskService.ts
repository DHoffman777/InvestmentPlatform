...
export class LiquidityRiskService {
  ...
  async assessLiquidityRisk(request: LiquidityRiskRequest): Promise<LiquidityRiskResult> {
    try {
      ...
      const positionLiquidity = await this.calculatePositionLiquidity(
        portfolioData,
        marketLiquidityData,
        request
      );
      ...
      const liquidityByAssetClass = await this.calculateLiquidityByCategory(
        portfolioData,
        positionLiquidity,
        'assetClass'
      );
      const liquidityBySector = await this.calculateLiquidityByCategory(
        portfolioData,
        positionLiquidity,
        'sector'
      );
      ...
      const liquidityUnderStress = await this.performLiquidityStressTesting(
        portfolioData,
        positionLiquidity,
        request
      );

      const result: LiquidityRiskResult = {
  ...
  // Calculate position-level liquidity metrics
  private async calculatePositionLiquidity(
    portfolioData: any[],
    marketLiquidityData: any,
    ...
    for (const position of portfolioData) {
      const liquidityMetrics = await this.assessPositionLiquidity(
        position,
        marketLiquidityData,
        request
      );
  ...
  private async assessPositionLiquidity(
    position: any,
    marketLiquidityData: any,
    request: LiquidityRiskRequest
  ): Promise<PositionLiquidity> {
    ...
    const liquidityCategory = await this.determineLiquidityCategory(
      daysToLiquidate,
      averageDailyVolume,
      bidAskSpread,
      position.assetClass
    ...
    const liquidationCost = await this.calculateLiquidationCost(
      position.marketValue,
      averageDailyVolume,
      bidAskSpread,
      daysToLiquidate,
      request.marketImpactModel
    );
  ...
  private async determineLiquidityCategory(
    daysToLiquidate: number,
    averageDailyVolume: number,
    bidAskSpread: number,
    assetClass: string
  ): Promise<LiquidityCategory> {
    ...
    if (daysToLiquidate <= 1) {
      return LiquidityCategory.IMMEDIATE;
  ...
  private getLiquidityThresholds(assetClass: string): any {
  ...
  private async calculateLiquidationCost(
    marketValue: number,
    averageDailyVolume: number,
    bidAskSpread: number,
    daysToLiquidate: number,
  ...
  private async calculateOverallLiquidityScore(
  ...
  private getPositionLiquidityScore(position: PositionLiquidity): number {
    // Score based on liquidity category
    const categoryScores: Record<LiquidityCategory, number> = {
      [LiquidityCategory.IMMEDIATE]: 95,
      [LiquidityCategory.HIGH]: 80,
      [LiquidityCategory.MEDIUM]: 60,
      [LiquidityCategory.LOW]: 35,
      [LiquidityCategory.ILLIQUID]: 10
    };
  ...
  private async calculateAverageDaysToLiquidate(
    positionLiquidity: PositionLiquidity[]
  ): Promise<number> {
    const totalValue = positionLiquidity.reduce((sum, pos) => sum + pos.marketValue, 0);
    let weightedDays = 0;
  ...
  private async calculateTotalLiquidationCost(
    positionLiquidity: PositionLiquidity[]
  ): Promise<number> {
  ...
  private async calculateTotalMarketImpact(
    positionLiquidity: PositionLiquidity[]
  ): Promise<number> {
    const totalValue = positionLiquidity.reduce((sum, pos) => sum + pos.marketValue, 0);
  ...
  private async calculateLiquidityByCategory(
    portfolioData: any[],
    positionLiquidity: PositionLiquidity[],
    category: 'assetClass' | 'sector'
    ...
    const breakdowns: LiquidityBreakdown[] = [];
  ...
  private async calculateLiquidityBySize(
    portfolioData: any[],
    positionLiquidity: PositionLiquidity[]
  ): Promise<LiquidityBreakdown[]> {
    const totalValue = positionLiquidity.reduce((sum, pos) => sum + pos.marketValue, 0);
    const sizeCategories = ['Large (>$1M)', 'Medium ($100K-$1M)', 'Small (<$100K)'];
    const sizeBuckets: Record<string, {
      value: number;
      liquidationDays: number[];
      liquidationCosts: number;
    }> = {};
  ...
  // Perform liquidity stress testing
  private async performLiquidityStressTesting(
    portfolioData: any[],
    positionLiquidity: PositionLiquidity[],
    request: LiquidityRiskRequest
    ...
    const stressResults: LiquidityStressResult[] = [];

    for (const scenario of stressScenarios) {
      const stressedLiquidity = await this.applyLiquidityStress(
        positionLiquidity,
        scenario,
        request
      );
  ...
  private async applyLiquidityStress(
    positionLiquidity: PositionLiquidity[],
    stressScenario: any,
    request: LiquidityRiskRequest
  ): Promise<PositionLiquidity[]> {
    return positionLiquidity.map(position => {
      ...
      const stressedDaysToLiquidate = Math.max(1, 
        Math.ceil(positionShares / maxDailyVolume) * stressScenario.liquidityMultiplier
      );
  ...
  private async getMarketLiquidityData(portfolioData: any[], asOfDate: Date): Promise<any> {
  ...
  private async estimateAverageDailyVolume(position: any): Promise<number> {
  ...
  private async estimateBidAskSpread(position: any): Promise<number> {
