...
export interface ReportColumn {
  ...
  source: string; // Database field or calculated field
...
export interface ReportFilter {
  ...
  minValue?: any; // For BETWEEN operator
  maxValue?: any; // For BETWEEN operator
}
...
export interface ReportGrouping {
  columnId: string;
  level: number; // 1 for top level, 2 for sub-groups, etc.
...
export interface ReportSection {
  ...
  showCondition?: string; // JavaScript expression
}
...
export interface ReportTemplate {
  ...
  dataSource: {
    baseEntity: string; // e.g., 'portfolio', 'position', 'transaction'
    joins?: string[]; // Related entities to join
    dateRange?: {
      type: 'FIXED' | 'RELATIVE' | 'PROMPT';
      startDate?: Date;
      endDate?: Date;
      relativePeriod?: string; // e.g., 'last_30_days', 'year_to_date'
    };
  };
