...
exports.InvestmentObjectivesService = void 0;
...
class InvestmentObjectivesService {
    ...
    async createInvestmentObjective(request, tenantId, userId) {
        try {
            ...
            if (existingObjective) {
                logger_1.logger.warn('Objective with same priority exists, adjusting priorities');
            ...
            logger_1.logger.info('Investment objective created successfully', {
                objectiveId: objective.id,
                clientId: request.clientId
            });
            return objective;
     ...
     */
    async createInvestmentRestriction(request, tenantId, userId) {
        try {
            ...
            logger_1.logger.info('Investment restriction created successfully', {
                restrictionId: restriction.id,
                clientId: request.clientId
            });
            return restriction;
     ...
     */
    async getClientObjectives(clientId, tenantId) {
        try {
     ...
     */
    async getClientRestrictions(clientId, tenantId, includeInactive = false) {
        try {
            logger_1.logger.info('Retrieving client restrictions', { clientId, tenantId, includeInactive });
     ...
     */
    async updateObjective(objectiveId, updates, tenantId, userId) {
        try {
            logger_1.logger.info('Updating investment objective', { objectiveId, tenantId });
            ...
            if (updates.priority && updates.priority !== existingObjective.priority) {
                await this.adjustObjectivePriorities(existingObjective.clientId, updates.priority, tenantId, objectiveId);
            }
     ...
     */
    async updateRestriction(restrictionId, updates, tenantId, userId) {
        try {
            logger_1.logger.info('Updating investment restriction', { restrictionId, tenantId });
            ...
            if (updates.isActive || updates.threshold || updates.appliesTo) {
                await this.triggerComplianceCheck(existingRestriction.clientId, tenantId, restrictionId);
            }
            return {
     ...
     */
    async deleteObjective(objectiveId, tenantId, userId) {
        try {
            logger_1.logger.info('Deleting investment objective', { objectiveId, tenantId });
            // Verify objective exists and belongs to tenant
            ...
            logger_1.logger.info('Investment objective deleted successfully', { objectiveId });
        }
        catch (error) {
     ...
     */
    async deleteRestriction(restrictionId, tenantId, userId) {
        try {
            logger_1.logger.info('Deleting investment restriction', { restrictionId, tenantId });
            // Verify restriction exists and belongs to tenant
            ...
            logger_1.logger.info('Investment restriction deleted successfully', { restrictionId });
        }
        catch (error) {
     ...
     */
    async analyzeObjectives(clientId, tenantId) {
        try {
            ...
            const totalTargetAllocation = objectives.reduce((sum, obj) => sum.plus(obj.targetAllocation || new library_1.Decimal(0)), new library_1.Decimal(0));
            ...
            const recommendations = this.generateObjectiveRecommendations(objectives, allocationAlignment, riskConsistency, timeHorizonRange);
            return {
                clientId,
     ...
     */
    async analyzeRestrictions(clientId, tenantId) {
        try {
            ...
            const impactAnalysis = await this.calculateRestrictionImpact(activeRestrictions, clientId, tenantId);
    ...
    async adjustObjectivePriorities(clientId, newPriority, tenantId, excludeObjectiveId) {
    ...
    async triggerSuitabilityReview(clientId, tenantId, trigger) {
    ...
    async triggerComplianceCheck(clientId, tenantId, restrictionId) {
    ...
    generateObjectiveRecommendations(objectives, allocationAlignment, riskConsistency, timeHorizonRange) {
    ...
    async calculateRestrictionImpact(restrictions, clientId, tenantId) {
        ...
        return {
            ...
            restrictedUniversePercentage: restrictedUniversePercentage.gt(100) ? new library_1.Decimal(100) : restrictedUniversePercentage,
            diversificationConstraints
        };
    }
    calculateComplianceScore(restrictions) {
        // Simplified compliance score calculation
        // In reality, this would check actual portfolio against restrictions
        if (restrictions.length === 0)
            return 100;
        const activeRestrictions = restrictions.filter(res => res.isActive);
        const criticalRestrictions = activeRestrictions.filter(res => res.violationAction === 'BLOCK');
...
exports.InvestmentObjectivesService = InvestmentObjectivesService;
