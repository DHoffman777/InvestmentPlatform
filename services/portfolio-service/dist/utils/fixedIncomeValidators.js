...
exports.FixedIncomeValidators = void 0;
class FixedIncomeValidators {
    static validateBond(bond) {
        const errors = [];
        const warnings = [];
        // Required fields
        if (!bond.bondId)
            errors.push('Bond ID is required');
        if (!bond.cusip)
            errors.push('CUSIP is required');
        if (!bond.issuerName)
            errors.push('Issuer name is required');
        if (!bond.bondType)
            errors.push('Bond type is required');
    ...
    static validateTreasuryBill(bill) {
        const errors = [];
        const warnings = [];
        // Required fields
        if (!bill.billId)
    ...
    static validateFixedIncomePosition(request) {
        const errors = [];
        const warnings = [];
        // Required fields
        ...
        if (request.purchasePrice === undefined)
            errors.push('Purchase price is required');
        if (request.faceValue === undefined)
            errors.push('Face value is required');
    ...
    static validateFixedIncomeTrade(request) {
        const errors = [];
        const warnings = [];
        // Required fields
        if (!request.portfolioId)
    ...
    static validateCouponPayment(request) {
        const errors = [];
        const warnings = [];
        // Required fields
        ...
        if (request.couponRate === undefined)
            errors.push('Coupon rate is required');
        if (!request.tenantId)
            errors.push('Tenant ID is required');
        // Coupon rate validation
        if (request.couponRate !== undefined) {
    ...
    static validateYieldCalculation(yieldData) {
        const errors = [];
        const warnings = [];
        // Required data for yield calculation
        if (yieldData.currentPrice === undefined)
            errors.push('Current price is required for yield calculation');
        if (yieldData.faceValue === undefined)
            errors.push('Face value is required for yield calculation');
        if (yieldData.couponRate === undefined)
            errors.push('Coupon rate is required for yield calculation');
        if (!yieldData.maturityDate)
            errors.push('Maturity date is required for yield calculation');
        // Value validations
    ...
    static validateMaturityEvent(maturityData) {
        const errors = [];
        const warnings = [];
        // Required fields
        if (!maturityData.positionId)
            errors.push('Position ID is required');
        if (!maturityData.maturityDate)
            errors.push('Maturity date is required');
        if (maturityData.principalAmount === undefined)
    ...
    static validateCUSIPCheckDigit(cusip) {
        if (cusip.length !== 9)
            return false;
...
exports.FixedIncomeValidators = FixedIncomeValidators;
