...
import { LoadTestExecutor } from './services/LoadTestExecutor';
...
export class LoadTestingService extends EventEmitter {
  ...
  constructor(private config: LoadTestingServiceConfig) {
    super();
    this.app = express();
  ...
  private setupCronJobs(): void {
    ...
    if (this.config.scheduling.cleanupSchedule) {
      cron.schedule(this.config.scheduling.cleanupSchedule, async () => {
  ...
  private validateTestConfig(config: LoadTestConfig): { valid: boolean; issues: string[] } {
    const issues: string[] = [];
  ...
  private async getStoredTestResult(testId: string): Promise<LoadTestResult | null> {
  ...
  private async listTests(status?: string, limit: number = 20, offset: number = 0): Promise<any[]> {
  ...
  private async scheduleTest(config: LoadTestConfig, schedule: string): Promise<string> {
    const scheduleId = `schedule_${Date.now()}`;
    
    // Parse cron schedule and validate
    try {
      const timeout = cron.schedule(schedule, async () => {
      ...
      timeout.start();
      this.scheduledTests.set(scheduleId, timeout as any);
  ...
  private async executeBenchmark(config: BenchmarkConfig): Promise<string> {
    ...
    try {
      ...
      for (const target of config.targets) {
        const targetResults: LoadTestResult[] = [];
  ...
  private compareBenchmarkResults(results: BenchmarkResult['results'], metrics: string[]): BenchmarkResult['comparison'] {
    const improvements: BenchmarkResult['comparison']['improvements'] = [];
    const regressions: BenchmarkResult['comparison']['regressions'] = [];
    let winner: string | undefined;

    if (results.length < 2) return { improvements, regressions, winner };
  ...
  private extractMetricValue(result: BenchmarkResult['results'][0], metric: string): number | null {
    if (result.loadTestResults.length === 0) return null;
  ...
  private evaluateBenchmarkPassing(result: BenchmarkResult, criteria: BenchmarkConfig['passingCriteria']): boolean {
  ...
  private generateBenchmarkSummary(result: BenchmarkResult): string {
  ...
  private getTestTemplates(): LoadTestConfig[] {
    return [
      {
  ...
  public start(port: number = 3010): void {
