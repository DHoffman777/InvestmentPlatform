export interface FinancialServicesFirm {
  ...
  crd: string; // CRD number
  iard: string; // IARD number
  seic: string; // SEC Investment Company number
...
export interface FinancialServicesConfig {
  ...
  compliance: {
    ...
    reportingDeadlines: {
      routine: number; // days
      material: number; // hours
      immediate: number; // minutes
    };
    reviewCycles: {
      suitability: number; // days
      bestExecution: number; // days
      aml: number; // days
      trading: number; // days
    };
  };
