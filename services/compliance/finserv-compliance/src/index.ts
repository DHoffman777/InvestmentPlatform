...
import { FinancialServicesComplianceService } from './FinancialServicesComplianceService';
import { FinancialServicesConfig } from './types';

const config: FinancialServicesConfig = {
  service: {
  ...
  regulators: {
    sec: {
      ...
      filingSystem: (process.env.SEC_FILING_SYSTEM as 'EDGAR' | 'IARD') || 'EDGAR',
...
const service = new FinancialServicesComplianceService(config);
