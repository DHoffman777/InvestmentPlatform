...
export interface EndpointAnalysis {
  ...
  trends: {
    responseTimeChange: number; // percentage change over time
...
export class ApiPerformanceOptimizer extends EventEmitter {
  ...
  private readonly maxMetricsPerEndpoint = 10000;
  ...
  private checkForAlerts(endpointKey: string, metrics: ApiPerformanceMetrics): void {
  ...
  private createAlert(endpointKey: string, alertType: PerformanceAlert['alertType'], options: {
    message: string;
    currentValue: number;
    threshold: number;
  ...
  private getRecommendedActions(alertType: PerformanceAlert['alertType']): string[] {
    switch (alertType) {
      case 'SLOW_RESPONSE':
        return [
  ...
  public analyzeEndpoint(endpoint: string, method: string, timeWindow: number = 3600000): EndpointAnalysis | null {
  ...
  private percentile(sortedArray: number[], percentile: number): number {
    ...
    if (sortedArray[lower] === undefined) return 0;
    if (sortedArray[upper] === undefined) return sortedArray[lower];

    return sortedArray[lower] * (1 - weight) + sortedArray[upper] * weight;
  }

  private calculateTrends(endpointKey: string, timeWindow: number): EndpointAnalysis['trends'] {
    const endpointMetrics = this.metrics.get(endpointKey) || [];
    const cutoffTime = new Date(Date.now() - timeWindow);
    const halfWindowTime = new Date(Date.now() - timeWindow / 2);
    
    const olderMetrics = endpointMetrics.filter(m => 
      m.timestamp >= cutoffTime && m.timestamp < halfWindowTime
    );
    const newerMetrics = endpointMetrics.filter(m => 
      m.timestamp >= halfWindowTime
    );
  ...
  private generateOptimizationRecommendations(metrics: ApiPerformanceMetrics[]): EndpointAnalysis['optimization'] {
    const avgResponseTime = metrics.reduce((sum, m) => sum + m.responseTime, 0) / metrics.length;
    const p95ResponseTime = this.percentile(
      metrics.map(m => m.responseTime).sort((a, b) => a - b),
      0.95
    );
  ...
  public createOptimization(optimization: Omit<PerformanceOptimization, 'id' | 'createdAt' | 'status'>): string {
  ...
  public updateOptimizationStatus(id: string, status: PerformanceOptimization['status'], metrics?: {
    avgResponseTime: number;
    p95ResponseTime: number;
    throughput: number;
  }): boolean {
  ...
  public getOptimizationsByPriority(): PerformanceOptimization[] {
  ...
  public generatePerformanceReport(timeWindow: number = 86400000): { // 24 hours default
    summary: {
  ...
  public generateAutomaticOptimizations(): PerformanceOptimization[] {
    ...
    const createdOptimizations: PerformanceOptimization[] = [];
  ...
  public getMetricsForEndpoint(endpoint: string, method: string): ApiPerformanceMetrics[] {
    const endpointKey = `${method} ${endpoint}`;
    return this.metrics.get(endpointKey) || [];
  }

  public clearMetrics(endpoint?: string, method?: string): void {
    if (endpoint && method) {
      const endpointKey = `${method} ${endpoint}`;
