...
export class DatabaseMonitor extends EventEmitter {
  ...
  private async collectHealthMetrics(): Promise<DatabaseHealth> {
    const [
      connectionStats,
      performanceStats,
      resourceStats,
  ...
  private async getResourceStats(): Promise<DatabaseHealth['resources']> {
    try {
      // These would typically come from system monitoring tools
      // For now, we'll provide estimated values based on database metrics
      const dbSize = await this.prisma.$queryRaw<Array<{ size: number }>>`
  ...
  private async getLockStats(): Promise<DatabaseHealth['locks']> {
    try {
      const lockStats = await this.prisma.$queryRaw<Array<{
        mode: string;
        count: number;
      }>>`
      ...
      const waitingQueries = await this.prisma.$queryRaw<Array<{ count: number }>>`
        SELECT count(*)::int as count
        FROM pg_stat_activity
  ...
  private async getReplicationStats(): Promise<DatabaseHealth['replication']> {
    try {
      // Check if we're on a replica
      const replicationInfo = await this.prisma.$queryRaw<Array<{
        in_recovery: boolean;
      }>>`SELECT pg_is_in_recovery() as in_recovery;`;

      if (!replicationInfo[0].in_recovery) {
        // We're on the primary, check replica lag
        const replicationLag = await this.prisma.$queryRaw<Array<{
          lag: number;
        }>>`
          SELECT COALESCE(
  ...
  private calculateOverallHealth(health: DatabaseHealth): 'HEALTHY' | 'WARNING' | 'CRITICAL' {
    const issues: string[] = [];

    // Check various health indicators
    if (health.performance.avgQueryTime > 1000) issues.push('Slow queries detected');
    if (health.connectionPool.waiting > 5) issues.push('Connection pool congestion');
    if (health.performance.cacheHitRatio < 0.9) issues.push('Low cache hit ratio');
    if (health.resources.cpuUsage > 80) issues.push('High CPU usage');
    if (health.resources.memoryUsage > 85) issues.push('High memory usage');
    if (health.resources.diskUsage > 90) issues.push('High disk usage');
    if (health.locks.waitingQueries > 10) issues.push('Lock contention');
    if (health.replication.replicationHealth === 'CRITICAL') issues.push('Replication issues');
  ...
  private checkAlertRules(health: DatabaseHealth): void {
  ...
  private getMetricValue(health: DatabaseHealth, metricPath: string): number {
  ...
  private takeAutomatedActions(alert: PerformanceAlert): void {
  ...
  public async getDetailedStatistics(): Promise<DatabaseStatistics> {
  ...
  private async getConnectionStatistics() {
    ...
    const maxConnections = await this.prisma.$queryRaw<Array<{ max_connections: number }>>`
  ...
  public getHealthHistory(hours: number = 24): DatabaseHealth[] {
