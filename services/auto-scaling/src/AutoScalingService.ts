...
import { ScalingDecisionEngine } from './services/ScalingDecisionEngine';
import { ScalingExecutor } from './services/ScalingExecutor';
...
export class AutoScalingService extends EventEmitter {
  ...
  private initializeServices(): void {
    ...
    this.decisionEngine = new ScalingDecisionEngine(this.config, this.financialProfile);
    this.scalingExecutor = new ScalingExecutor(this.config);
  }
  ...
  private setupRoutes(): void {
    ...
    this.app.post('/api/v1/reports/generate', async (req, res) => {
      try {
        ...
        const report = await this.generateScalingReport(
          new Date(startDate),
          new Date(endDate),
          services
        );
  ...
  private async getScalingStatus(): Promise<any> {
    const allMetrics = this.metricsCollector.getAllMetrics();
    const activeScalings = this.scalingExecutor.getActiveScalings();
  ...
  private async generateScalingReport(
    ...
    // Analyze by service
    const serviceAnalysis = new Map<string, any>();
    ...
    const serviceAnalysisArray = Array.from(serviceAnalysis.values()).map(analysis => ({
      ...
      averageInstances: analysis.instanceCounts.reduce((sum: number, count: number) => sum + count, 0) / analysis.instanceCounts.length,
  ...
  public async start(port: number = 3011): Promise<void> {
    ...
    if (this.config.reporting.enabled && this.config.reporting.schedule) {
      this.reportingSchedule = cron.schedule(this.config.reporting.schedule, async () => {
        try {
