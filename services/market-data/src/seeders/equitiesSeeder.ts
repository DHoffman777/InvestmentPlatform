...
const preferredStocks: PreferredStockData[] = [
  {
    symbol: 'BAC-PL',
...
export async function seedEquities() {
  logger.info('Starting equities seeding process...');

  try {
    // Seed common stocks
    logger.info(`Seeding ${commonStocks.length} common stocks...`);
    for (const stock of commonStocks) {
      await equitiesService.upsertEquity(stock);
      logger.info(`✓ Seeded common stock: ${stock.symbol} - ${stock.name}`);
    }

    // Seed preferred stocks
    logger.info(`Seeding ${preferredStocks.length} preferred stocks...`);
    for (const stock of preferredStocks) {
      await equitiesService.upsertEquity(stock);
      logger.info(`✓ Seeded preferred stock: ${stock.symbol} - ${stock.name}`);
    }

    // Seed ADRs
    logger.info(`Seeding ${adrs.length} ADRs...`);
    for (const adr of adrs) {
      await equitiesService.upsertEquity(adr);
      logger.info(`✓ Seeded ADR: ${adr.symbol} - ${adr.name}`);
    }

    logger.info('Equities seeding completed successfully!');
...
if (require.main === module) {
  seedEquities()
    .then(() => {
      logger.info('Equities seeding script completed');
      process.exit(0);
    })
    .catch((error) => {
      logger.error('Equities seeding script failed:', error);
      process.exit(1);
    })
    .finally(async () => {
