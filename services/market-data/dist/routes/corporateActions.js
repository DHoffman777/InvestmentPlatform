...
exports.corporateActionsRouter = void 0;
...
exports.corporateActionsRouter = router;
...
router.get('/:symbol', [
    ...
    (0, express_validator_1.query)('actionType').optional().isIn(['DIVIDEND', 'SPLIT', 'MERGER', 'SPINOFF', 'SPECIAL_DIVIDEND', 'RIGHTS_OFFERING']).withMessage('Invalid action type'),
    ...
    try {
        ...
        let corporateActions = await marketDataService.getCorporateActions(symbol.toUpperCase(), startDate, endDate);
        ...
        logger_1.logger.error('Error fetching corporate actions:', {
...
router.post('/', [
    ...
    (0, express_validator_1.body)('actionType').isIn(['DIVIDEND', 'SPLIT', 'MERGER', 'SPINOFF', 'SPECIAL_DIVIDEND', 'RIGHTS_OFFERING']).withMessage('Invalid action type'),
    (0, express_validator_1.body)('exDate').isISO8601().toDate().withMessage('Ex-date is required and must be valid'),
    (0, express_validator_1.body)('recordDate').optional().isISO8601().toDate().withMessage('Record date must be valid'),
    (0, express_validator_1.body)('payDate').optional().isISO8601().toDate().withMessage('Pay date must be valid'),
    (0, express_validator_1.body)('announcementDate').optional().isISO8601().toDate().withMessage('Announcement date must be valid'),
    (0, express_validator_1.body)('effectiveDate').optional().isISO8601().toDate().withMessage('Effective date must be valid'),
    (0, express_validator_1.body)('description').isString().trim().isLength({ min: 1, max: 500 }).withMessage('Description is required and must be 1-500 characters'),
    (0, express_validator_1.body)('value').optional().isNumeric().withMessage('Value must be numeric'),
    (0, express_validator_1.body)('ratio').optional().isString().trim().withMessage('Ratio must be a string'),
    ...
    try {
        ...
        logger_1.logger.error('Error creating corporate action:', { corporateActionData: req.body, error });
        res.status(500).json({
            error: 'Internal server error',
...
router.get('/', [
    (0, express_validator_1.query)('daysAhead').optional().isInt({ min: 0, max: 365 }).toInt().withMessage('Days ahead must be between 0 and 365'),
    (0, express_validator_1.query)('actionType').optional().isIn(['DIVIDEND', 'SPLIT', 'MERGER', 'SPINOFF', 'SPECIAL_DIVIDEND', 'RIGHTS_OFFERING']).withMessage('Invalid action type'),
    ...
    try {
        const { daysAhead = 30, actionType, page = 1, limit = 20 } = req.query;
        ...
        const [corporateActions, total] = await Promise.all([
            database_1.prisma.corporateAction.findMany({
                where: whereClause,
                skip,
                take: limit,
                include: {
            ...
            database_1.prisma.corporateAction.count({ where: whereClause })
        ]);
        const formattedActions = corporateActions.map(action => ({
            ...action,
            value: action.value?.toNumber(),
        ...
        logger_1.logger.error('Error listing corporate actions:', { query: req.query, error });
        res.status(500).json({
            error: 'Internal server error',
