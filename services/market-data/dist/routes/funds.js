...
router.get('/search', [
    ...
    try {
        const filters = {
            ...
            investmentStyle: req.query.investmentStyle,
            marketCapFocus: req.query.marketCapFocus,
            geographicFocus: req.query.geographicFocus,
            fundFamily: req.query.fundFamily,
        ...
        logger_1.logger.error('Error searching funds:', { filters: req.query, error });
        res.status(500).json({
...
router.get('/:symbol', [
    ...
    try {
        ...
        logger_1.logger.error('Error fetching fund details:', { symbol: req.params.symbol, error });
        res.status(500).json({
            error: 'Internal server error',
...
router.post('/etf', [
    ...
    try {
        ...
        logger_1.logger.error('Error creating/updating ETF:', { etfData: req.body, error });
        res.status(500).json({
            error: 'Internal server error',
...
router.post('/mutual-fund', [
    ...
    try {
        ...
        logger_1.logger.error('Error creating/updating mutual fund:', { fundData: req.body, error });
        res.status(500).json({
            error: 'Internal server error',
...
router.get('/families', auth_1.authenticateJWT, (0, auth_1.requirePermission)(['market-data:read']), async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error fetching fund families:', error);
        res.status(500).json({
            error: 'Internal server error',
...
router.get('/categories', auth_1.authenticateJWT, (0, auth_1.requirePermission)(['market-data:read']), async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error fetching fund categories:', error);
        res.status(500).json({
            error: 'Internal server error',
