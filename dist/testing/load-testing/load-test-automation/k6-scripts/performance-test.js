...
exports.marketDataConsumption = marketDataConsumption;
exports.clientPortalUsage = clientPortalUsage;
exports.reportingAnalytics = reportingAnalytics;
...
const bundle_js_1 = require("https://raw.githubusercontent.com/benc-uk/k6-reporter/main/dist/bundle.js");
/**
 * K6 Performance Test Script for Investment Management Platform
 * Comprehensive load testing with business-specific scenarios
 */
// Custom metrics
const orderPlacementRate = new metrics_1.Rate('order_placement_success');
const portfolioUpdateRate = new metrics_1.Rate('portfolio_update_success');
const marketDataLatency = new metrics_1.Trend('market_data_latency');
const tradingErrors = new metrics_1.Counter('trading_errors');
// Test data
...
const symbols = new data_1.SharedArray('symbols', function () {
    return ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA', 'META', 'NVDA', 'NFLX', 'SPY', 'QQQ'];
});
const portfolioIds = new data_1.SharedArray('portfolios', function () {
...
function authenticate(user) {
    const loginResponse = http_1.default.post(`${BASE_URL}/api/auth/login`, JSON.stringify({
...
function portfolioManagement() {
    ...
    const dashboardResponse = http_1.default.get(`${BASE_URL}/api/portfolios/dashboard`, { headers });
    const dashboardEnd = new Date().getTime();
    (0, k6_1.check)(dashboardResponse, {
    ...
    const positionsResponse = http_1.default.get(`${BASE_URL}/api/portfolios/${portfolioId}/positions`, { headers });
    (0, k6_1.check)(positionsResponse, {
        'positions loaded': (r) => r.status === 200,
        'positions data present': (r) => r.json('positions') !== null,
    });
    // Get portfolio performance
    const performanceResponse = http_1.default.get(`${BASE_URL}/api/portfolios/${portfolioId}/performance`, { headers });
    (0, k6_1.check)(performanceResponse, {
        'performance loaded': (r) => r.status === 200,
        'performance metrics present': (r) => r.json('returns') !== null,
    });
    (0, k6_1.sleep)(1); // Think time
}
...
function tradingOperations() {
    ...
    const orderResponse = http_1.default.post(`${BASE_URL}/api/orders`, JSON.stringify(orderData), { headers });
    const orderSuccess = (0, k6_1.check)(orderResponse, {
    ...
    if (orderSuccess) {
        ...
        // Check order status
        const statusResponse = http_1.default.get(`${BASE_URL}/api/orders/${orderId}/status`, { headers });
        (0, k6_1.check)(statusResponse, {
    ...
    // Get active orders
    const activeOrdersResponse = http_1.default.get(`${BASE_URL}/api/orders/active`, { headers });
    (0, k6_1.check)(activeOrdersResponse, {
        'active orders retrieved': (r) => r.status === 200,
    });
    (0, k6_1.sleep)(0.5); // Short think time for trading
}
// Market data consumption scenario
function marketDataConsumption() {
    const user = users[Math.floor(Math.random() * users.length)];
    const token = authenticate(user);
    if (!token)
        return;
    ...
    const quotesResponse = http_1.default.get(`${BASE_URL}/api/market-data/quotes/bulk?symbols=${symbols.join(',')}`, { headers });
    const quotesEnd = new Date().getTime();
    marketDataLatency.add(quotesEnd - quotesStart);
    (0, k6_1.check)(quotesResponse, {
        'quotes retrieved': (r) => r.status === 200,
        'quotes have data': (r) => r.json('quotes').length > 0,
    });
    // Get level 2 data for random symbol
    const symbol = symbols[Math.floor(Math.random() * symbols.length)];
    const level2Response = http_1.default.get(`${BASE_URL}/api/market-data/level2/${symbol}`, { headers });
    (0, k6_1.check)(level2Response, {
        'level2 data retrieved': (r) => r.status === 200,
        'level2 has bids/asks': (r) => r.json('bids') && r.json('asks'),
    });
    // Get intraday chart
    const chartResponse = http_1.default.get(`${BASE_URL}/api/market-data/charts/intraday/${symbol}`, { headers });
    (0, k6_1.check)(chartResponse, {
        'chart data retrieved': (r) => r.status === 200,
    });
    (0, k6_1.sleep)(0.1); // Fast market data consumption
}
// Client portal usage scenario
function clientPortalUsage() {
    ...
    const summaryResponse = http_1.default.get(`${BASE_URL}/api/client/portfolio-summary`, { headers });
    (0, k6_1.check)(summaryResponse, {
        'portfolio summary loaded': (r) => r.status === 200,
        'summary has total value': (r) => r.json('totalValue') !== null,
    });
    // Get recent transactions
    const transactionsResponse = http_1.default.get(`${BASE_URL}/api/client/recent-transactions`, { headers });
    (0, k6_1.check)(transactionsResponse, {
        'transactions loaded': (r) => r.status === 200,
    });
    // Get client alerts
    const alertsResponse = http_1.default.get(`${BASE_URL}/api/client/alerts`, { headers });
    (0, k6_1.check)(alertsResponse, {
        'alerts loaded': (r) => r.status === 200,
    });
    // Get statements
    const statementsResponse = http_1.default.get(`${BASE_URL}/api/client/statements`, { headers });
    (0, k6_1.check)(statementsResponse, {
        'statements loaded': (r) => r.status === 200,
    });
    (0, k6_1.sleep)(3); // Longer think time for client users
}
...
function reportingAnalytics() {
    ...
    const performanceResponse = http_1.default.get(`${BASE_URL}/api/reports/performance`, { headers });
    (0, k6_1.check)(performanceResponse, {
        'performance report generated': (r) => r.status === 200,
    });
    // Get risk analytics
    const riskResponse = http_1.default.get(`${BASE_URL}/api/analytics/risk`, { headers });
    (0, k6_1.check)(riskResponse, {
    ...
    const customReportResponse = http_1.default.post(`${BASE_URL}/api/reports/generate`, JSON.stringify(reportData), { headers });
    (0, k6_1.check)(customReportResponse, {
        'custom report requested': (r) => r.status === 202,
    });
    (0, k6_1.sleep)(2); // Think time for analysis
}
...
function setup() {
    ...
    const healthResponse = http_1.default.get(`${BASE_URL}/health`);
...
function handleSummary(data) {
    return {
        'load-test-results/k6-performance-report.html': (0, bundle_js_1.htmlReport)(data),
