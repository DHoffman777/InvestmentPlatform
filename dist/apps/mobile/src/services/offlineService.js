"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.offlineService = void 0;
const react_native_encrypted_storage_1 = __importDefault(require("react-native-encrypted-storage"));
const netinfo_1 = __importDefault(require("@react-native-community/netinfo"));
const index_1 = require("@store/index");
const networkSlice_1 = require("@store/slices/networkSlice");
const apiClient_1 = require("./apiClient");
class OfflineService {
    static instance;
    isOnline = true;
    syncInProgress = false;
    OFFLINE_ACTIONS_KEY = 'offline_actions';
    CACHED_DATA_KEY = 'cached_data';
    MAX_RETRY_COUNT = 3;
    CACHE_TTL = {
        portfolio: 5 * 60 * 1000, // 5 minutes
        positions: 3 * 60 * 1000, // 3 minutes
        transactions: 10 * 60 * 1000, // 10 minutes
        marketData: 1 * 60 * 1000, // 1 minute
        userProfile: 30 * 60 * 1000, // 30 minutes
    };
    constructor() {
        this.initializeNetworkListener();
    }
    static getInstance() {
        if (!OfflineService.instance) {
            OfflineService.instance = new OfflineService();
        }
        return OfflineService.instance;
    }
    // Initialize network state monitoring
    initializeNetworkListener() {
        netinfo_1.default.addEventListener(state => {
            const wasOffline = !this.isOnline;
            this.isOnline = state.isConnected && state.isInternetReachable;
            index_1.store.dispatch((0, networkSlice_1.setNetworkStatus)({
                isConnected: state.isConnected,
                isInternetReachable: state.isInternetReachable,
                type: state.type,
            }));
            index_1.store.dispatch((0, networkSlice_1.setOfflineMode)(!this.isOnline));
            // Sync when coming back online
            if (wasOffline && this.isOnline) {
                this.syncOfflineActions();
            }
        });
    }
    // Check if device is online
    isDeviceOnline() {
        return this.isOnline;
    }
    // Queue an action for offline execution
    async queueOfflineAction(endpoint, method, data) {
        const action = {
            id: `${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
            type: this.getActionType(method),
            endpoint,
            method,
            data,
            timestamp: Date.now(),
            retryCount: 0,
        };
        const existingActions = await this.getOfflineActions();
        const updatedActions = [...existingActions, action];
        await react_native_encrypted_storage_1.default.setItem(this.OFFLINE_ACTIONS_KEY, JSON.stringify(updatedActions));
        return action.id;
    }
    // Get all queued offline actions
    async getOfflineActions() {
        try {
            const actionsJson = await react_native_encrypted_storage_1.default.getItem(this.OFFLINE_ACTIONS_KEY);
            return actionsJson ? JSON.parse(actionsJson) : [];
        }
        catch (error) {
            console.error('Error getting offline actions:', error);
            return [];
        }
    }
    // Sync offline actions when online
    async syncOfflineActions() {
        if (!this.isOnline || this.syncInProgress) {
            return;
        }
        this.syncInProgress = true;
        const actions = await this.getOfflineActions();
        const successfulActions = [];
        const failedActions = [];
        console.log(`Syncing ${actions.length} offline actions...`);
        for (const action of actions) {
            try {
                await this.executeOfflineAction(action);
                successfulActions.push(action.id);
                console.log(`Successfully synced action: ${action.id}`);
            }
            catch (error) {
                action.retryCount++;
                if (action.retryCount < this.MAX_RETRY_COUNT) {
                    failedActions.push(action);
                    console.log(`Retrying action ${action.id} (attempt ${action.retryCount})`);
                }
                else {
                    console.error(`Failed to sync action ${action.id} after ${this.MAX_RETRY_COUNT} attempts`);
                }
            }
        }
        // Update offline actions queue
        await react_native_encrypted_storage_1.default.setItem(this.OFFLINE_ACTIONS_KEY, JSON.stringify(failedActions));
        this.syncInProgress = false;
        console.log(`Sync completed. ${successfulActions.length} successful, ${failedActions.length} pending retry`);
    }
    // Execute a single offline action
    async executeOfflineAction(action) {
        const { endpoint, method, data } = action;
        switch (method) {
            case 'POST':
                await apiClient_1.apiClient.post(endpoint, data);
                break;
            case 'PUT':
                await apiClient_1.apiClient.put(endpoint, data);
                break;
            case 'PATCH':
                await apiClient_1.apiClient.patch(endpoint, data);
                break;
            case 'DELETE':
                await apiClient_1.apiClient.delete(endpoint);
                break;
        }
    }
    // Cache data for offline access
    async cacheData(key, data, category = 'portfolio') {
        const cachedItem = {
            key,
            data,
            timestamp: Date.now(),
            ttl: this.CACHE_TTL[category],
        };
        try {
            const existingCache = await this.getCachedData();
            const updatedCache = {
                ...existingCache,
                [key]: cachedItem,
            };
            await react_native_encrypted_storage_1.default.setItem(this.CACHED_DATA_KEY, JSON.stringify(updatedCache));
        }
        catch (error) {
            console.error('Error caching data:', error);
        }
    }
    // Get cached data
    async getCachedDataByKey(key) {
        try {
            const cache = await this.getCachedData();
            const item = cache[key];
            if (!item) {
                return null;
            }
            // Check if cache is expired
            const isExpired = Date.now() - item.timestamp > item.ttl;
            if (isExpired) {
                await this.removeCachedData(key);
                return null;
            }
            return item.data;
        }
        catch (error) {
            console.error('Error getting cached data:', error);
            return null;
        }
    }
    // Get all cached data
    async getCachedData() {
        try {
            const cacheJson = await react_native_encrypted_storage_1.default.getItem(this.CACHED_DATA_KEY);
            return cacheJson ? JSON.parse(cacheJson) : {};
        }
        catch (error) {
            console.error('Error getting cached data:', error);
            return {};
        }
    }
    // Remove specific cached data
    async removeCachedData(key) {
        try {
            const cache = await this.getCachedData();
            delete cache[key];
            await react_native_encrypted_storage_1.default.setItem(this.CACHED_DATA_KEY, JSON.stringify(cache));
        }
        catch (error) {
            console.error('Error removing cached data:', error);
        }
    }
    // Clear all cached data
    async clearCache() {
        try {
            await react_native_encrypted_storage_1.default.removeItem(this.CACHED_DATA_KEY);
        }
        catch (error) {
            console.error('Error clearing cache:', error);
        }
    }
    // Clear expired cache entries
    async clearExpiredCache() {
        try {
            const cache = await this.getCachedData();
            const currentTime = Date.now();
            const validCache = {};
            Object.entries(cache).forEach(([key, item]) => {
                const isExpired = currentTime - item.timestamp > item.ttl;
                if (!isExpired) {
                    validCache[key] = item;
                }
            });
            await react_native_encrypted_storage_1.default.setItem(this.CACHED_DATA_KEY, JSON.stringify(validCache));
        }
        catch (error) {
            console.error('Error clearing expired cache:', error);
        }
    }
    // Get cache statistics
    async getCacheStats() {
        try {
            const cache = await this.getCachedData();
            const currentTime = Date.now();
            let totalSize = 0;
            let expiredItems = 0;
            Object.values(cache).forEach(item => {
                totalSize += JSON.stringify(item).length;
                const isExpired = currentTime - item.timestamp > item.ttl;
                if (isExpired) {
                    expiredItems++;
                }
            });
            return {
                totalItems: Object.keys(cache).length,
                totalSize,
                expiredItems,
            };
        }
        catch (error) {
            console.error('Error getting cache stats:', error);
            return { totalItems: 0, totalSize: 0, expiredItems: 0 };
        }
    }
    // Get pending offline actions count
    async getPendingActionsCount() {
        const actions = await this.getOfflineActions();
        return actions.length;
    }
    // Helper method to determine action type
    getActionType(method) {
        switch (method) {
            case 'POST':
                return 'CREATE';
            case 'PUT':
            case 'PATCH':
                return 'UPDATE';
            case 'DELETE':
                return 'DELETE';
            default:
                return 'UPDATE';
        }
    }
    // Initialize offline service on app start
    async initialize() {
        // Clear expired cache on initialization
        await this.clearExpiredCache();
        // Get initial network state
        const networkState = await netinfo_1.default.fetch();
        this.isOnline = networkState.isConnected && networkState.isInternetReachable;
        index_1.store.dispatch((0, networkSlice_1.setNetworkStatus)({
            isConnected: networkState.isConnected,
            isInternetReachable: networkState.isInternetReachable,
            type: networkState.type,
        }));
        index_1.store.dispatch((0, networkSlice_1.setOfflineMode)(!this.isOnline));
        // Sync any pending actions if online
        if (this.isOnline) {
            this.syncOfflineActions();
        }
    }
}
exports.offlineService = OfflineService.getInstance();
