 ...
 */
class ContainerSecurityScanner extends events_1.EventEmitter {
    scanResults = new Map();
    scanPolicies = new Map();
    knownVulnerabilities = new Map();
    registryCredentials = new Map();
    constructor() {
        super();
     ...
     */
    async scanImage(imageName, tag = 'latest', options = {}) {
        try {
     ...
     */
    async batchScan(images, options = {}) {
        const maxConcurrent = options.maxConcurrent || 5;
        const results = [];
        for (let i = 0; i < images.length; i += maxConcurrent) {
            const batch = images.slice(i, i + maxConcurrent);
            const batchPromises = batch.map(img => this.scanImage(img.name, img.tag, {
     ...
     */
    getImageScanHistory(imageName, limit = 10) {
     ...
     */
    generateSecurityReport(timeRange, filters) {
    ...
    async getImageMetadata(imageName, tag, registry) {
        ...
        return {
            ...
            digest: 'sha256:' + (0, crypto_1.randomUUID)().replace(/-/g, ''),
    ...
    async performVulnerabilityScan(image, deepScan = false) {
    ...
    async scanPackageManager(image, packageManager, deepScan) {
        const vulnerabilities = [];
    ...
    async performComplianceChecks(image) {
    ...
    updateVulnerabilitySummary(scanResult) {
        const summary = {
            critical: 0,
            high: 0,
            medium: 0,
    ...
    async applyPolicyChecks(scanResult, policyId) {
    ...
    async sendPolicyNotifications(policy, scanResult, violations) {
    ...
    filterScanResults(timeRange, filters) {
        return Array.from(this.scanResults.values())
            .filter(result => {
            ...
            if (filters?.severities && filters.severities.length > 0) {
                const hasSeverity = result.vulnerabilities.some(v => filters.severities.includes(v.severity));
                if (!hasSeverity)
                    return false;
            }
            if (filters?.imagePatterns && filters.imagePatterns.length > 0) {
                const matchesPattern = filters.imagePatterns.some((pattern) => result.imageName.includes(pattern));
                if (!matchesPattern)
                    return false;
            }
            return true;
    ...
    generateTrends(results, timeRange) {
        // Generate daily trends
        const trends = [];
        const msPerDay = 24 * 60 * 60 * 1000;
        for (let d = new Date(timeRange.start); d <= timeRange.end; d.setTime(d.getTime() + msPerDay)) {
            const dayResults = results.filter(r => r.scanDate.toDateString() === d.toDateString());
            trends.push({
                date: new Date(d),
    ...
    getTopRiskyImages(results, limit) {
        const imageRisks = new Map();
    ...
    calculateRiskScore(result) {
        ...
        return (result.summary.critical * weights.critical +
...
exports.default = ContainerSecurityScanner;
