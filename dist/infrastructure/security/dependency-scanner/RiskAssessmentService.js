...
class RiskAssessmentService extends events_1.EventEmitter {
    ...
    async assessRisk(dependency, vulnerabilities, tenantId, businessContext, assessedBy = 'system') {
    ...
    async calculateRiskFactors(dependency, vulnerabilities, context) {
        const factors = [];
    ...
    async calculateDependencyRiskFactors(dependency) {
        const factors = [];
    ...
    calculateEnvironmentalRiskFactors(context) {
        const factors = [];
        factors.push({
            id: this.generateFactorId(),
    ...
    calculateBusinessRiskFactors(context) {
        const factors = [];
        factors.push({
            id: this.generateFactorId(),
            category: 'BUSINESS',
    ...
    calculateBusinessImpactScore(factors, context) {
    ...
    calculateTechnicalRiskScore(factors, vulnerabilities) {
    ...
    calculateExploitabilityScore(vulnerabilities) {
    ...
    calculateEnvironmentalScore(context) {
    ...
    calculateOverallRiskScore(businessImpact, technicalRisk, exploitability, environmental) {
        ...
        return Math.round(businessImpact * weights.business +
    ...
    async generateMitigationStrategies(dependency, vulnerabilities, context, riskLevel) {
        const strategies = [];
        // Update strategy
    ...
    calculatePriority(riskScore, riskLevel, context) {
        ...
        if (context.applicationCriticality === 'CRITICAL')
            priority += 15;
        else if (context.applicationCriticality === 'HIGH')
            priority += 10;
    ...
    async prioritizeRiskAssessments(assessments, tenantId) {
    ...
    generatePriorityJustification(assessment, context) {
        const reasons = [];
    ...
    async assessMaintenanceStatus(dependency) {
    ...
    calculatePackageAgeScore(dependency) {
        ...
        if (majorVersion === 0)
            return 70; // Pre-1.0 versions are riskier
        if (majorVersion >= 3)
            return 20; // Mature versions
        return 40; // Moderate age
    }
    assessLicenseRisk(licenses) {
    ...
    determineUpdateCost(dependency, context) {
    ...
    determineUpdateTimeline(riskLevel, context) {
    ...
    determineUpdateFeasibility(dependency) {
        // Base feasibility
        let feasibility = 80;
    ...
    determineReplacementTimeline(context) {
        if (context.environmentType === 'PRODUCTION')
            return '4-8 weeks';
        return '2-4 weeks';
    }
    determineReplacementFeasibility(dependency) {
    ...
    hasConfigurationMitigation(vulnerabilities) {
        // Check if any vulnerabilities can be mitigated through configuration
        return vulnerabilities.some(vuln => vuln.description.toLowerCase().includes('configuration') ||
            vuln.description.toLowerCase().includes('setting') ||
            vuln.cwe?.some(cwe => ['CWE-16', 'CWE-15'].includes(cwe)));
    }
    getDefaultBusinessContext(tenantId) {
        return {
            tenantId,
    ...
    async getDependencyFromAssessment(assessment) {
    ...
    async getVulnerabilitiesFromAssessment(assessment) {
    ...
    // Public API methods
    setBusinessContext(tenantId, context) {
    ...
    updateAssessmentCriteria(criteria) {
    ...
    getHighRiskAssessments(tenantId) {
        const assessments = tenantId
            ? this.getAssessmentsByTenant(tenantId)
            : Array.from(this.assessments.values());
        return assessments.filter(a => a.riskLevel === 'CRITICAL' || a.riskLevel === 'HIGH').sort((a, b) => b.priority - a.priority);
    }
    async reassessRisk(assessmentId) {
