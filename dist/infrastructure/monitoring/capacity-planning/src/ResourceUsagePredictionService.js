...
exports.ResourceUsagePredictionService = void 0;
const events_1 = require("events");
const CapacityPlanningDataModel_1 = require("./CapacityPlanningDataModel");
class ResourceUsagePredictionService extends events_1.EventEmitter {
    models = new Map();
    predictions = new Map();
    ...
    async createPredictionModel(modelConfig) {
        const model = {
            ...
            type: modelConfig.type || CapacityPlanningDataModel_1.ModelType.TIME_SERIES,
            resourceType: modelConfig.resourceType,
            metric: modelConfig.metric,
            algorithm: modelConfig.algorithm || CapacityPlanningDataModel_1.PredictionAlgorithm.LINEAR_REGRESSION,
    ...
    async generatePrediction(modelId, resourceId, options = {}) {
        const model = this.models.get(modelId);
        if (!model) {
            throw new Error(`Model ${modelId} not found`);
        }
    ...
    async batchPredict(requests) {
        ...
        for (const batch of batches) {
            const batchPromises = batch.map(request => this.generatePrediction(request.modelId, request.resourceId, request.options)
                .catch(error => {
    ...
    async getModelsByResourceType(resourceType) {
        return Array.from(this.models.values()).filter(model => model.resourceType === resourceType && model.isActive);
    }
    async getBestModelForResource(resourceId, metric) {
        ...
        return metricModels.reduce((best, current) => current.accuracy > best.accuracy ? current : best);
    }
    async preprocessData(data, model) {
    ...
    async executeTraining(model, data) {
        switch (model.algorithm) {
            case CapacityPlanningDataModel_1.PredictionAlgorithm.LINEAR_REGRESSION:
                return this.trainLinearRegression(data, model.parameters);
            case CapacityPlanningDataModel_1.PredictionAlgorithm.ARIMA:
                return this.trainARIMA(data, model.parameters);
            case CapacityPlanningDataModel_1.PredictionAlgorithm.EXPONENTIAL_SMOOTHING:
                return this.trainExponentialSmoothing(data, model.parameters);
            case CapacityPlanningDataModel_1.PredictionAlgorithm.PROPHET:
                return this.trainProphet(data, model.parameters);
            default:
                return this.trainLinearRegression(data, model.parameters);
        }
    }
    async executePrediction(model, data, horizon) {
    ...
    trainLinearRegression(data, parameters) {
        const n = data.length;
    ...
    predictLinearRegression(data, horizon, parameters) {
        const n = data.length;
        const x = Array.from({ length: n }, (_, i) => i);
        const sumX = x.reduce((sum, val) => sum + val, 0);
        const sumY = data.reduce((sum, val) => sum + val, 0);
    ...
    calculatePointConfidence(value, historicalData) {
    ...
    detectAnomaly(value, historicalData) {
        const mean = historicalData.reduce((sum, val) => sum + val, 0) / historicalData.length;
        const stdDev = Math.sqrt(historicalData.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) / historicalData.length);
    ...
    generateModelRecommendations(model, metrics, trend) {
    ...
    generateParameterCombinations(model) {
        const combinations = [];
    ...
    async getHistoricalData(resourceId, metric, lookbackPeriod) {
    ...
    async getResourceType(resourceId) {
        return CapacityPlanningDataModel_1.ResourceType.SERVER;
    }
    generateMockData(resourceId, metric, start, end) {
        const data = [];
        ...
        const defaultModels = [
            {
                resourceType: CapacityPlanningDataModel_1.ResourceType.SERVER,
                metric: 'cpu_usage',
                algorithm: CapacityPlanningDataModel_1.PredictionAlgorithm.LINEAR_REGRESSION
            },
            {
                resourceType: CapacityPlanningDataModel_1.ResourceType.SERVER,
                metric: 'memory_usage',
                algorithm: CapacityPlanningDataModel_1.PredictionAlgorithm.EXPONENTIAL_SMOOTHING
            },
            {
                resourceType: CapacityPlanningDataModel_1.ResourceType.DATABASE,
                metric: 'cpu_usage',
                algorithm: CapacityPlanningDataModel_1.PredictionAlgorithm.ARIMA
            }
        ];
    ...
    startModelRetraining() {
        this.retrainingTimer = setInterval(async () => {
            const modelsToRetrain = Array.from(this.models.values()).filter(model => model.isActive &&
                model.nextTraining &&
...
exports.ResourceUsagePredictionService = ResourceUsagePredictionService;
