...
exports.CapacityTrendAnalyzer = void 0;
const events_1 = require("events");
const CapacityPlanningDataModel_1 = require("./CapacityPlanningDataModel");
class CapacityTrendAnalyzer extends events_1.EventEmitter {
    trends = new Map();
    analysisTimer;
    config;
    timeSeriesProcessor;
    ...
    async batchAnalyzeTrends(requests) {
        ...
        for (const batch of batches) {
            const batchPromises = batch.map(request => this.analyzeTrend(request).catch(error => {
    ...
    async detectAnomalies(resourceId, metric, timeRange) {
        const request = {
            resourceId,
            metric,
            timeRange: {
                ...
                granularity: CapacityPlanningDataModel_1.TimeGranularity.HOUR
            }
        };
    ...
    async compareResourceTrends(resourceIds, metric, timeRange) {
    ...
    async getTrendSummary(resourceType, timeRange) {
        let trends = Array.from(this.trends.values());
        if (timeRange) {
            trends = trends.filter(t => t.timeRange.start >= timeRange.start && t.timeRange.end <= timeRange.end);
        }
    ...
    async decomposeTrend(timeSeries) {
    ...
    async extractSeasonal(detrended) {
        const period = 24;
    ...
    extractTimeSeries(metrics, metric) {
        const values = [];
        const timestamps = [];
    ...
    determineTrendDirection(trendValues) {
    ...
    async generateRecommendations(trendComponents, seasonality, changePoints, request) {
        ...
        if (changePoints.length > 0) {
            const recentChangePoints = changePoints.filter(cp => Date.now() - cp.timestamp.getTime() < 7 * 24 * 60 * 60 * 1000);
    ...
    async detectTimeSeriesAnomalies(timeSeries, trend) {
        const anomalies = [];
    ...
    async compareTrends(trends) {
        const correlations = [];
        for (let i = 0; i < trends.length; i++) {
            for (let j = i + 1; j < trends.length; j++) {
                const correlation = this.calculateCorrelation([trends[i].trend.slope], [trends[j].trend.slope]);
                correlations.push({
    ...
    getIntervalForGranularity(granularity) {
        switch (granularity) {
            case CapacityPlanningDataModel_1.TimeGranularity.MINUTE:
                return 60 * 1000;
            case CapacityPlanningDataModel_1.TimeGranularity.HOUR:
                return 60 * 60 * 1000;
            case CapacityPlanningDataModel_1.TimeGranularity.DAY:
                return 24 * 60 * 60 * 1000;
            case CapacityPlanningDataModel_1.TimeGranularity.WEEK:
                return 7 * 24 * 60 * 60 * 1000;
            case CapacityPlanningDataModel_1.TimeGranularity.MONTH:
                return 30 * 24 * 60 * 60 * 1000;
            default:
                return 60 * 60 * 1000;
        }
    }
    generateMockMetrics(resourceId, timestamp) {
    ...
    generateTrendId(resourceId, metric) {
...
exports.CapacityTrendAnalyzer = CapacityTrendAnalyzer;
class TimeSeriesProcessor {
    async smooth(values, windowSize = 5) {
        const smoothed = [];
        for (let i = 0; i < values.length; i++) {
            const start = Math.max(0, i - Math.floor(windowSize / 2));
            const end = Math.min(values.length, i + Math.floor(windowSize / 2) + 1);
    ...
    async detectOutliers(values, threshold = 2) {
        const mean = values.reduce((sum, val) => sum + val, 0) / values.length;
        const stdDev = Math.sqrt(values.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) / values.length);
        return values.map((val, index) => Math.abs(val - mean) > threshold * stdDev ? index : -1).filter(index => index !== -1);
    }
}
class SeasonalityDetector {
    ...
    async analyzePattern(values, period) {
...
class ChangePointDetector {
    ...
    classifyChangeType(meanChange, varChange, changePercent) {
...
class ForecastEngine {
    ...
    async generate(timeSeries, seasonality) {
        const values = timeSeries.values;
        if (values.length < 5) {
