...
exports.SLACustomerNotificationService = exports.NotificationType = void 0;
const events_1 = require("events");
const SLADataModel_1 = require("./SLADataModel");
var NotificationType;
(function (NotificationType) {
    NotificationType["BREACH_NOTIFICATION"] = "breach_notification";
    NotificationType["BREACH_RESOLVED"] = "breach_resolved";
    NotificationType["MAINTENANCE_NOTICE"] = "maintenance_notice";
    NotificationType["COMPLIANCE_REPORT"] = "compliance_report";
    NotificationType["SLA_ACHIEVEMENT"] = "sla_achievement";
    NotificationType["THRESHOLD_WARNING"] = "threshold_warning";
    NotificationType["SERVICE_DEGRADATION"] = "service_degradation";
    NotificationType["PLANNED_OUTAGE"] = "planned_outage";
    NotificationType["EMERGENCY_ALERT"] = "emergency_alert";
    NotificationType["MONTHLY_SUMMARY"] = "monthly_summary";
})(NotificationType || (exports.NotificationType = NotificationType = {}));
class SLACustomerNotificationService extends events_1.EventEmitter {
    config;
    customers = new Map();
    ...
    async sendBreachNotification(customerId, breach, slaDefinition) {
        const request = {
            customerId,
        ...
        if (this.config.escalationMatrix.automaticEscalation && breach.severity === SLADataModel_1.SLASeverity.CRITICAL) {
            await this.startEscalation(breach.id, customerId, breach.severity);
        }
        return notificationId;
    }
    async sendResolutionNotification(customerId, breach, slaDefinition) {
        ...
        return this.sendCustomerNotification({
            ...
            severity: SLADataModel_1.SLASeverity.INFO,
    ...
    async sendComplianceReport(customerId, report, complianceScore) {
        return this.sendCustomerNotification({
            customerId,
    ...
    async sendMaintenanceNotification(customerId, maintenanceWindow) {
        ...
        return this.sendCustomerNotification({
            ...
            severity: SLADataModel_1.SLASeverity.MEDIUM,
    ...
    async acknowledgeNotification(customerId, notificationId) {
        const history = this.notificationHistory.get(customerId);
        if (!history) {
            throw new Error(`Customer ${customerId} not found`);
        }
    ...
    async unsubscribeCustomer(customerId, category) {
        const customer = this.customers.get(customerId);
        if (!customer) {
            throw new Error(`Customer ${customerId} not found`);
        }
        if (category && category.length > 0) {
            ...
            customer.preferences.categories = customer.preferences.categories.filter(cat => !category.includes(cat));
        }
        else {
            // Full unsubscribe
    ...
    async deliverNotification(request) {
        const customer = this.customers.get(request.customerId);
        if (!customer)
            return;
        const template = this.getTemplate(request.type, customer.preferences.language);
    ...
    async sendViaChannel(channel, customer, content, request) {
        const startTime = Date.now();
        switch (channel) {
            case SLADataModel_1.SLANotificationChannel.EMAIL:
                return this.sendEmail(customer.email, content.subject, content.html, content.text);
            case SLADataModel_1.SLANotificationChannel.SMS:
                if (customer.phone) {
                    return this.sendSMS(customer.phone, content.text);
                }
                throw new Error('No phone number available for SMS');
            case SLADataModel_1.SLANotificationChannel.SLACK:
                return this.sendSlackMessage(customer, content);
            case SLADataModel_1.SLANotificationChannel.WEBHOOK:
    ...
    async sendSlackMessage(customer, content) {
        // Mock Slack sending
    ...
    async sendWebhook(customer, request, content) {
    ...
    shouldNotifyCustomer(customer, request) {
    ...
    getTemplate(type, language) {
        ...
        if (this.templates.has(langSpecificKey)) {
            return this.templates.get(langSpecificKey);
        }
        // Fallback to default language
        return this.templates.get(type) || null;
    }
    recordDeliveryResult(customerId, request, channel, result) {
        const history = this.notificationHistory.get(customerId);
        if (!history)
            return;
        const notification = history.notifications.find(n => n.type === request.type &&
    ...
    addToHistory(customerId, notification) {
    ...
    async startEscalation(incidentId, customerId, severity) {
    ...
    async escalateIncident(incidentId, escalation) {
    ...
    getMaintenancePreparationSteps() {
        return [
...
exports.SLACustomerNotificationService = SLACustomerNotificationService;
class RateLimiter {
    config;
    hourlyCount = 0;
    dailyCount = 0;
