...
exports.SLAComplianceScoringService = void 0;
const events_1 = require("events");
const SLADataModel_1 = require("./SLADataModel");
class SLAComplianceScoringService extends events_1.EventEmitter {
    scores = new Map();
    config;
    ...
    async calculateComplianceScore(context) {
    ...
    async calculateScoreComponents(context) {
        const components = [];
    ...
    async calculateAvailabilityScore(context) {
        const availabilityMetrics = context.metrics.filter(m => m.unit === '%' && m.currentValue <= 100);
    ...
    async calculatePerformanceScore(context) {
        const performanceMetrics = context.metrics.filter(m => m.unit === 'ms' || m.unit === 'seconds' || m.unit === 'rps');
    ...
    async calculateReliabilityScore(context) {
    ...
    async calculateBreachImpactScore(context) {
        let score = 100;
        const factors = [];
        ...
        if (breaches.length === 0) {
            ...
            for (const [severity, count] of Object.entries(severityDistribution)) {
                const multiplier = this.config.penalties.severityMultipliers[severity] || 1;
            ...
            const recentBreaches = breaches.filter(b => b.startTime.getTime() > (Date.now() - (7 * 24 * 60 * 60 * 1000)));
    ...
    async calculateBusinessContextScore(context) {
        let score = 100;
        const factors = [];
    ...
    calculateScoreBreakdown(components) {
        const metricScores = {};
    ...
    async calculateScoreTrends(context) {
        const trends = [];
    ...
    generateScoreRecommendations(breakdown, trends, context) {
        ...
        if (context.breaches.length > 0) {
            const criticalBreaches = context.breaches.filter(b => b.severity === SLADataModel_1.SLASeverity.CRITICAL);
    ...
    async getComplianceGrade(score) {
        let grade;
        let description;
        const recommendations = [];
        if (score >= 97) {
    ...
    async getBenchmarkComparison(slaId, score) {
    ...
    async getHistoricalScores(slaId, timeRange) {
        const allScores = this.scores.get(slaId) || [];
        return allScores.filter(score => score.calculatedAt >= timeRange.start &&
            score.calculatedAt <= timeRange.end);
    }
    getBusinessContextMultiplier(businessContext, component) {
    ...
    calculateConfidence(dataPoints, timeWindow) {
        ...
        if (dataPoints >= 24)
            confidence += 0.15; // Daily coverage bonus
        if (dataPoints >= 168)
            confidence += 0.05; // Weekly coverage bonus
    ...
    createDefaultComponent(name, defaultScore, factors) {
        const weight = this.config.weights[name] || 0.2;
        return {
            name,
            weight,
...
exports.SLAComplianceScoringService = SLAComplianceScoringService;
