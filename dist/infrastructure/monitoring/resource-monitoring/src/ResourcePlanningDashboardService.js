...
exports.ResourcePlanningDashboardService = exports.WidgetType = void 0;
const events_1 = require("events");
var WidgetType;
(function (WidgetType) {
    WidgetType["UTILIZATION_GAUGE"] = "utilization_gauge";
    WidgetType["COST_TREND"] = "cost_trend";
    WidgetType["EFFICIENCY_SCORE"] = "efficiency_score";
    WidgetType["RESOURCE_ALLOCATION"] = "resource_allocation";
    WidgetType["OPTIMIZATION_OPPORTUNITIES"] = "optimization_opportunities";
    WidgetType["ALERT_SUMMARY"] = "alert_summary";
    WidgetType["CAPACITY_FORECAST"] = "capacity_forecast";
    WidgetType["PERFORMANCE_METRICS"] = "performance_metrics";
    WidgetType["COST_BREAKDOWN"] = "cost_breakdown";
    WidgetType["WASTE_ANALYSIS"] = "waste_analysis";
    WidgetType["SLA_COMPLIANCE"] = "sla_compliance";
    WidgetType["RESOURCE_TOPOLOGY"] = "resource_topology";
    WidgetType["RECOMMENDATION_LIST"] = "recommendation_list";
    WidgetType["CUSTOM_CHART"] = "custom_chart";
    WidgetType["DATA_TABLE"] = "data_table";
    WidgetType["KPI_CARD"] = "kpi_card";
})(WidgetType || (exports.WidgetType = WidgetType = {}));
class ResourcePlanningDashboardService extends events_1.EventEmitter {
    config;
    ...
    async generateDashboard(dashboardId, resourceData, costData, efficiencyData, allocationData, alerts, recommendations, filters) {
        ...
        for (const [widgetId, config] of this.widgetConfigs) {
            try {
                const widgetData = await this.generateWidgetData(config, resourceData, costData, efficiencyData, allocationData, alerts, recommendations, filters);
        ...
        const summary = this.generateDashboardSummary(resourceData, costData, efficiencyData, alerts, recommendations);
        ...
        const dashboardRecommendations = this.processDashboardRecommendations(recommendations, costData, filters);
        const dashboardData = {
            timestamp,
    ...
    async generateWidgetData(config, resourceData, costData, efficiencyData, allocationData, alerts, recommendations, filters) {
        const timestamp = new Date();
        let data = null;
        switch (config.type) {
    ...
    generateUtilizationGaugeData(resourceData, filters) {
        const filteredResources = this.filterResources(Array.from(resourceData.values()), filters);
        if (filteredResources.length === 0) {
            return { value: 0, status: 'no_data' };
        }
        const totalUtilization = filteredResources.reduce((sum, resource) => sum + resource.utilization.overall, 0);
    ...
    generateCostTrendData(costData, filters) {
        ...
        const trendData = timePoints.map(timestamp => {
            const totalCost = filteredCostData.reduce((sum, correlation) => {
                ...
                const forecast = correlation.forecast.predictions.find(p => Math.abs(p.date.getTime() - timestamp.getTime()) < 86400000 // Within 24 hours
                );
    ...
    generateEfficiencyScoreData(resourceData, efficiencyData, filters) {
        const filteredResources = this.filterResources(Array.from(resourceData.values()), filters);
        if (filteredResources.length === 0) {
            return { score: 0, status: 'no_data' };
        }
        const efficiencyScores = filteredResources.map(resource => resource.efficiency?.score || 0);
    ...
    generateResourceAllocationData(allocationData, filters) {
    ...
    generateOptimizationOpportunitiesData(recommendations, costData, filters) {
        ...
        const topOpportunities = [
            ...
            ...costOpportunities.slice(0, 5).map(opp => ({
                ...
                priority: opp.priority > 80 ? 'high' : 'medium',
                title: opp.title,
        ...
        const totalSavings = topOpportunities.reduce((sum, opp) => sum + (opp.impact.cost_savings || 0), 0);
        return {
    ...
    generateAlertSummaryData(alerts, filters) {
    ...
    generateDashboardSummary(resourceData, costData, efficiencyData, alerts, recommendations) {
        ...
        const averageEfficiency = resources.length > 0
            ...
            : 0;
        const potentialSavings = allCostData.reduce((sum, correlation) => sum + correlation.optimization_opportunities.reduce((opSum, opp) => opSum + opp.savings_amount, 0), 0) + allRecommendations.reduce((sum, rec) => sum + (rec.impact.cost_savings || 0), 0);
        return {
    ...
    filterCostData(costData, filters) { return costData; }
    filterAllocations(allocations, filters) { return allocations; }
    filterRecommendations(recommendations, filters) { return recommendations; }
    filterAlerts(alerts, filters) { return alerts; }
    generateTimePoints(filters) { return [new Date()]; }
    isForecastPoint(timestamp) { return false; }
    calculateUtilizationTrend(resources) { return 'stable'; }
    calculateCostTrend(trendData) { return 'stable'; }
    generateCostBreakdown(costData) { return {}; }
    calculateTotalSavingsOpportunities(costData) { return 0; }
    calculateForecastAccuracy(costData) { return 0.85; }
    getEfficiencyGrade(score) { return score > 0.8 ? 'A' : score > 0.6 ? 'B' : 'C'; }
    calculateImprovementPotential(insights) { return 25; }
    getBenchmarkComparison(score) { return { industry_average: 0.75, percentile: 60 }; }
    calculateAllocationUtilization(allocations) { return {}; }
    calculateAllocationCosts(allocations) { return {}; }
    calculateAllocationWaste(allocations) { return {}; }
    categorizeOpportunities(opportunities) { return {}; }
    calculateROIAnalysis(opportunities) { return {}; }
    calculateAlertTrends(alerts) { return {}; }
    getTopAlertTypes(alerts) { return {}; }
    calculateResolutionMetrics(alerts) { return {}; }
    determineCostTrend(costData) { return 'stable'; }
    calculateDataFreshness(resourceData) { return new Date(); }
    calculateDataQuality(widgets) { return 0.95; }
    identifyMissingData(resourceData, costData) { return []; }
    collectErrors(widgets) { return []; }
    countDataPoints(data) { return 100; }
    assessDataQuality(data) { return 0.95; }
    calculateFreshnessScore(data, timestamp) { return 1.0; }
    processDashboardAlerts(alerts, filters) { return []; }
    processDashboardRecommendations(recommendations, costData, filters) { return []; }
    generateKPICardData(config, resourceData, costData, efficiencyData, filters) { return {}; }
    generateCapacityForecastData(resourceData, filters) { return {}; }
    generatePerformanceMetricsData(resourceData, filters) { return {}; }
    generateWasteAnalysisData(resourceData, efficiencyData, filters) { return {}; }
    generateRecommendationListData(recommendations, filters) { return {}; }
    exportToJSON(dashboardData, config) { return JSON.stringify(dashboardData, null, 2); }
    exportToCSV(dashboardData, config) { return 'csv data'; }
    exportToExcel(dashboardData, config) { return Buffer.from('excel data'); }
    exportToPDF(dashboardData, config) { return Buffer.from('pdf data'); }
    exportToPNG(dashboardData, config) { return Buffer.from('png data'); }
    async shutdown() {
...
exports.ResourcePlanningDashboardService = ResourcePlanningDashboardService;
