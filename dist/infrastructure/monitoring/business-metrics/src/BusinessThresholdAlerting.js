...
exports.BusinessThresholdAlerting = void 0;
const events_1 = require("events");
const BusinessMetricsDataModel_1 = require("./BusinessMetricsDataModel");
class BusinessThresholdAlerting extends events_1.EventEmitter {
    alertRules = new Map();
    activeAlerts = new Map();
    alertHistory = new Map();
    alertStatistics = new Map();
    ...
    async createAlertRule(rule) {
        const newRule = {
            ...
            severity: rule.severity || BusinessMetricsDataModel_1.AlertSeverity.MEDIUM,
    ...
    async evaluateMetricValue(metricValue) {
        const evaluations = [];
    ...
    async evaluateKPI(kpiId, currentValue, context) {
        const evaluations = [];
        const relevantRules = Array.from(this.alertRules.values())
    ...
    async evaluateRule(rule, metricValue) {
    ...
    async evaluateKPIRule(rule, currentValue, context) {
        const conditionEvaluations = [];
        for (const condition of rule.conditions) {
    ...
    async evaluateCondition(condition, metricValue, rule) {
        ...
        if (condition.dimensions) {
            const dimensionMatch = Object.entries(condition.dimensions).every(([key, value]) => metricValue.dimensions[key] === value);
    ...
    async evaluateKPICondition(condition, currentValue, context) {
    ...
    async evaluatePatternCondition(metricId, condition) {
    ...
    async handleTriggeredAlert(rule, evaluation, metricValue) {
    ...
    async handleTriggeredKPIAlert(rule, evaluation, currentValue, context) {
        if (!await this.shouldTriggerAlert(rule, evaluation)) {
            return;
        }
        const alert = {
    ...
    async shouldTriggerAlert(rule, evaluation) {
        const existingAlert = Array.from(this.activeAlerts.values())
            .find(alert => (alert.metricId === evaluation.metricId || alert.kpiTargetId === evaluation.kpiId) &&
            alert.status === 'active');
        if (existingAlert) {
    ...
    async isSuppressed(suppressionRule, evaluation) {
        if (!suppressionRule.isActive)
            return false;
        switch (suppressionRule.type) {
            ...
            case 'dependency_based':
                ...
                return dependentAlerts.some((alertId) => this.activeAlerts.has(alertId));
            default:
                return false;
        }
    }
    ...
    async sendNotification(alert, channelId, rule) {
    ...
    async renderNotificationMessage(template, alert, rule) {
        const variables = {
            alertId: alert.id,
            ruleName: rule.name,
            severity: alert.severity,
    ...
    async resolveAlert(alertId, reason, userId) {
    ...
    async generateAlertMessage(rule, evaluation, metricValue) {
    ...
    async generateKPIAlertMessage(rule, evaluation, currentValue, context) {
        const condition = evaluation.conditions.find(c => c.isMet);
        if (!condition)
            return `KPI alert triggered for rule: ${rule.name}`;
    ...
    async setupAnomalyDetection(rule) {
        if (!rule.metricId)
            return;
        const config = {
            algorithm: 'zscore',
    ...
    async getPreviousValue(metricId) {
        return Math.random() * 1000;
    }
    async calculateRate(metricId, timeWindow) {
        return Math.random() * 10;
    }
    getSeverityColor(severity) {
        switch (severity) {
            case BusinessMetricsDataModel_1.AlertSeverity.LOW: return '#36a64f';
            case BusinessMetricsDataModel_1.AlertSeverity.MEDIUM: return '#ff9800';
            case BusinessMetricsDataModel_1.AlertSeverity.HIGH: return '#f44336';
            case BusinessMetricsDataModel_1.AlertSeverity.CRITICAL: return '#d32f2f';
            default: return '#9e9e9e';
        }
    }
    updateAlertStatistics(entityId, action) {
    ...
    initializeAlertStatistics(entityId) {
    ...
    recordAlertHistory(alertId, status, context) {
    ...
    async performPeriodicEvaluations() {
    ...
    async checkForMissingData(rule) {
        if (!rule.metricId)
            return;
...
exports.BusinessThresholdAlerting = BusinessThresholdAlerting;
class AnomalyDetector {
    ...
    addTrainingData(value) {
        this.trainingData.push(value);
        if (this.trainingData.length > 1000) {
            this.trainingData.shift();
        }
    }
    async isAnomaly(value) {
