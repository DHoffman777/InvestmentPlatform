...
exports.CashEquivalentValidators = void 0;
class CashEquivalentValidators {
    static validateMoneyMarketFund(fund) {
        const errors = [];
        const warnings = [];
        // Required fields
        if (!fund.fundId)
        ...
        if (!fund.symbol)
            errors.push('Fund symbol is required');
        if (!fund.provider)
            errors.push('Fund provider is required');
        if (!fund.category)
            errors.push('Fund category is required');
        // Category validation
    ...
    static validateSweepAccount(account) {
        const errors = [];
        const warnings = [];
        // Required fields
    ...
    static validateCashEquivalentPosition(positionData) {
        const errors = [];
        const warnings = [];
        // Required fields
        if (!positionData.portfolioId)
            errors.push('Portfolio ID is required');
        if (!positionData.tenantId)
            errors.push('Tenant ID is required');
        if (!positionData.assetType)
            errors.push('Asset type is required');
        if (!positionData.assetId)
    ...
    static validateSweepTransaction(transactionData) {
        const errors = [];
        const warnings = [];
        // Required fields
    ...
    static validateYieldDistribution(distributionData) {
        const errors = [];
        const warnings = [];
        // Required fields
        if (!distributionData.positionId)
            errors.push('Position ID is required');
        if (!distributionData.distributionDate)
            errors.push('Distribution date is required');
        if (distributionData.amount === undefined)
            errors.push('Amount is required');
        if (distributionData.yieldRate === undefined)
            errors.push('Yield rate is required');
        // Amount validation
...
exports.CashEquivalentValidators = CashEquivalentValidators;
