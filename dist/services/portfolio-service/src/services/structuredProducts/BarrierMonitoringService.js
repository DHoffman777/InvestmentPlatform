...
exports.BarrierMonitoringService = void 0;
const logger_1 = require("../../utils/logger");
class BarrierMonitoringService {
    prisma;
    kafkaService;
    ...
    async monitorBarriers(request) {
        try {
            logger_1.logger.info('Starting barrier monitoring', {
            ...
            for (const product of products) {
                if (product.hasBarrier && product.barriers) {
                    for (const barrier of product.barriers) {
                        if (barrier.isActive) {
                            totalBarriers++;
                            const barrierStatus = await this.evaluateBarrier(product, barrier, marketData, request.alertThreshold || 0.1);
                            ...
                            const barrierAlerts = await this.generateBarrierAlerts(product, barrier, barrierStatus, request.alertThreshold || 0.1);
            ...
            logger_1.logger.info('Barrier monitoring completed', {
                totalBarriers,
                activeBarriers: totalBarriers - hitBarriers,
                approachingBarriers,
                hitBarriers,
    ...
    async getProductsToMonitor(request) {
        try {
            ...
            if (request.productIds && request.productIds.length > 0) {
                filteredProducts = filteredProducts.filter(p => request.productIds.includes(p.id));
            }
            ...
            logger_1.logger.error('Error getting products to monitor:', error);
            throw error;
        }
    }
    // Get current market data for underlying assets
    async getCurrentMarketData(products) {
        try {
            const marketData = {};
    ...
    async evaluateBarrier(product, barrier, marketData, alertThreshold) {
        try {
            ...
            const hasBeenHit = this.checkBarrierHit(barrier.barrierType, currentLevel, barrierLevel);
            ...
            logger_1.logger.error('Error evaluating barrier:', { barrierId: barrier.id, error });
            throw error;
        }
    }
    // Check if barrier has been hit
    checkBarrierHit(barrierType, currentLevel, barrierLevel) {
        switch (barrierType) {
            case 'DOWN_AND_OUT':
            case 'DOWN_AND_IN':
    ...
    async generateBarrierAlerts(product, barrier, status, alertThreshold) {
        const alerts = [];
        try {
            ...
            logger_1.logger.error('Error generating barrier alerts:', error);
            return [];
        }
    }
    // Calculate alert severity based on distance to barrier
    calculateAlertSeverity(distancePercentage, alertThreshold) {
    ...
    isRecoveryScenario(barrierType) {
    ...
    async updateBarrierHitStatus(barrierId, hitLevel) {
        try {
            // In a real implementation, this would update the database
            logger_1.logger.info('Barrier hit detected', {
                barrierId,
                hitLevel,
                hitTime: new Date()
            });
            ...
            logger_1.logger.error('Error updating barrier hit status:', error);
        }
    }
    // Store monitoring results
    async storeMonitoringResults(barrierStatuses, alerts) {
        try {
            // In a real implementation, this would store in database
            logger_1.logger.debug('Storing monitoring results', {
                barrierCount: barrierStatuses.length,
                alertCount: alerts.length
            });
        }
        catch (error) {
            logger_1.logger.error('Error storing monitoring results:', error);
        }
    }
    // Publish barrier alert to Kafka
    async publishBarrierAlert(alert) {
        try {
            ...
            logger_1.logger.info('Barrier alert published', {
                alertId: alert.id,
                alertType: alert.alertType,
                severity: alert.severity
            });
            ...
            logger_1.logger.error('Error publishing barrier alert:', error);
        }
    }
    // Real-time barrier monitoring (called periodically)
    async performRealTimeMonitoring(intervalMinutes = 5) {
        logger_1.logger.info('Starting real-time barrier monitoring', { intervalMinutes });
        const monitor = async () => {
            try {
                const request = {
                ...
                logger_1.logger.debug('Real-time monitoring cycle completed', {
                    totalBarriers: result.summary.totalBarriers,
                    newAlerts: result.alerts.length
                });
            }
            catch (error) {
                logger_1.logger.error('Error in real-time monitoring cycle:', error);
            }
        };
        // Initial monitoring
        await monitor();
    ...
    async getBarrierHistory(productId, startDate, endDate) {
        try {
            ...
            logger_1.logger.error('Error getting barrier history:', error);
            throw error;
        }
    }
    // Calculate barrier breach probability using statistical models
    async calculateBreachProbability(productId, timeHorizonDays, confidenceLevel = 0.95) {
        try {
            ...
            logger_1.logger.error('Error calculating breach probability:', error);
            throw error;
        }
    }
    ...
    async getBarrierDashboard(tenantId) {
        try {
            // Get all products for tenant
            const request = { alertThreshold: 0.1 };
            const monitoringResult = await this.monitorBarriers(request);
            // Calculate risk scores for products
            const productRiskScores = new Map();
            ...
            const recentAlerts = monitoringResult.alerts.filter(alert => Date.now() - alert.alertTime.getTime() < 24 * 60 * 60 * 1000);
            return {
                summary: {
            ...
            logger_1.logger.error('Error getting barrier dashboard:', error);
            throw error;
        }
    }
    // Calculate risk score for a barrier
    calculateRiskScore(barrier) {
        let riskScore = 0;
...
exports.BarrierMonitoringService = BarrierMonitoringService;
