...
class DataExtractionService {
    ...
    async extractData(request) {
        try {
            ...
            let extractedFields = [];
            let extractionMethod = 'HYBRID';
            if (request.template) {
                processingSteps.push('Template-based extraction');
                const templateFields = await this.performTemplateBasedExtraction(request.ocrResults, request.template, request.language);
            ...
            if (request.customExtractionRules && request.customExtractionRules.length > 0) {
                ...
                const customFields = await this.performCustomRulesExtraction(request.ocrResults, request.customExtractionRules, request.language);
                extractedFields = this.mergeExtractedFields(extractedFields, customFields);
            }
            processingSteps.push('NLP extraction');
            const nlpFields = await this.performNLPExtraction(request.ocrResults, request.language);
            extractedFields = this.mergeExtractedFields(extractedFields, nlpFields);
            processingSteps.push('ML-based extraction');
            const mlFields = await this.performMLBasedExtraction(request.ocrResults, request.language);
    ...
    async performTemplateBasedExtraction(ocrResults, template, language) {
    ...
    async performCustomRulesExtraction(ocrResults, rules, language) {
        const extractedFields = [];
    ...
    async extractFieldUsingRule(ocrResults, rule, language) {
    ...
    extractUsingRegex(ocrResults, rule) {
    ...
    extractUsingOCRRegion(ocrResults, rule) {
        if (!rule.coordinates)
            return null;
        for (const ocrResult of ocrResults) {
    ...
    async extractUsingNLP(ocrResults, rule, language) {
        if (!this.nlpEngine)
            return null;
        try {
            const fullText = ocrResults.map(r => r.text).join(' ');
            const nlpResult = await this.nlpEngine.process(fullText, language);
            const relevantEntities = nlpResult.entities.filter((entity) => entity.label.toLowerCase().includes(rule.fieldName.toLowerCase()) ||
                rule.pattern && new RegExp(rule.pattern, 'i').test(entity.text));
            if (relevantEntities.length > 0) {
                const bestEntity = relevantEntities.sort((a, b) => b.confidence - a.confidence)[0];
                const alternatives = relevantEntities.slice(1).map((entity) => ({
                    value: entity.text,
    ...
    async extractUsingMLModel(ocrResults, rule, language) {
        try {
    ...
    async performNLPExtraction(ocrResults, language) {
        const extractedFields = [];
        if (!this.nlpEngine)
            return extractedFields;
        try {
            ...
            for (const field of commonFields) {
                const entities = nlpResult.entities.filter((e) => field.labels.includes(e.label.toUpperCase()));
                if (entities.length > 0) {
                    const bestEntity = entities.sort((a, b) => b.confidence - a.confidence)[0];
                    const alternatives = entities.slice(1, 4).map((e) => ({
                        value: e.text,
    ...
    async performMLBasedExtraction(ocrResults, language) {
        const extractedFields = [];
        try {
            ...
            for (const fieldName of defaultModel.supportedFields) {
                ...
                const prediction = await this.predictFieldValue(defaultModel, features, this.inferFieldType(fieldName));
    ...
    async performPostProcessing(fields, language) {
    ...
    async performValidation(fields, template) {
    ...
    async validateField(field, rule) {
        try {
    ...
    async performBasicValidation(field) {
        const results = [];
    ...
    mergeExtractedFields(existing, additional) {
    ...
    processFieldValue(value, fieldType) {
        try {
            switch (fieldType) {
                case 'NUMBER':
    ...
    isWithinBounds(regionBox, targetBox) {
    ...
    mapExtractionMethodToSource(method) {
        switch (method) {
            case 'NLP': return 'NLP';
    ...
    extractMLFeatures(text, fieldName) {
        const features = [];
    ...
    async predictFieldValue(model, features, fieldType) {
        try {
    ...
    async executeCustomValidation(field, rule) {
        try {
    ...
    async initializeNLPEngine() {
        try {
            this.nlpEngine = {
                process: async (text, language) => ({
    ...
    async initializeFieldProcessors() {
    ...
    async publishDataExtractionEvent(documentId, tenantId, result) {
        const event = {
