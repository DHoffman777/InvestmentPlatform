...
exports.MachineLearningInsightsService = void 0;
...
class MachineLearningInsightsService {
    ...
    async generateInsights(request) {
        try {
            logger_1.logger.info('Generating ML insights', {
            ...
            logger_1.logger.error('Error generating ML insights:', error);
            throw error;
        }
    }
    async performClusterAnalysis(data, features, numClusters) {
        try {
            logger_1.logger.info('Performing cluster analysis', { dataPoints: data.length, features: features.length });
            ...
            logger_1.logger.error('Error performing cluster analysis:', error);
            throw error;
        }
    }
    async recognizePatterns(data, patternTypes = ['trend', 'reversal', 'breakout', 'consolidation']) {
        try {
            logger_1.logger.info('Recognizing patterns', { dataPoints: data.length, patternTypes });
            const patterns = [];
            ...
            logger_1.logger.error('Error recognizing patterns:', error);
            throw error;
        }
    }
    async generateOptimizationSuggestions(portfolioData, constraints = [], objective = 'sharpe') {
        try {
            logger_1.logger.info('Generating optimization suggestions', { objective, constraints: constraints.length });
            const suggestions = [];
            // Portfolio rebalancing optimization
            const rebalancingSuggestion = await this.generateRebalancingSuggestion(portfolioData, objective);
            if (rebalancingSuggestion)
            ...
            logger_1.logger.error('Error generating optimization suggestions:', error);
            throw error;
        }
    }
    async analyzePerformanceDrivers(portfolioData, benchmarkData, timeRange) {
        try {
            logger_1.logger.info('Analyzing performance drivers', { timeRange });
            // Factor analysis
            ...
            logger_1.logger.error('Error analyzing performance drivers:', error);
            throw error;
        }
    }
    async getInsightsByEntity(entityId, entityType, categories, minConfidence) {
        const insights = Array.from(this.insights.values()).filter(insight => {
            // Check entity match
            const entityMatch = (insight.entities.portfolios?.includes(entityId) && entityType === 'portfolio') ||
            ...
            if (minConfidence && insight.confidence < minConfidence)
                return false;
            // Only return valid insights
            if (insight.validUntil && insight.validUntil < new Date())
                return false;
            return true;
        });
    ...
    async markInsightActionTaken(insightId, action, takenBy, outcome) {
    ...
    async generateClusterAnalysisInsights(request) {
        const insights = [];
    ...
    async generatePatternRecognitionInsights(request) {
        const insights = [];
    ...
    async generateRiskAttributionInsights(request) {
        const insights = [];
    ...
    async generatePerformanceDriverInsights(request) {
        const insights = [];
    ...
    extractFeatureMatrix(data, features) {
        return data.map(point => features.map(feature => point.metadata?.[feature] || Math.random() * 100));
    }
    ...
    async performKMeansClustering(featureMatrix, k) {
        // Mock K-means clustering
        const assignments = featureMatrix.map(() => Math.floor(Math.random() * k));
        const centroids = Array.from({ length: k }, () => Array.from({ length: featureMatrix[0]?.length || 0 }, () => Math.random() * 100));
        return { assignments, centroids };
    }
    async analyzeClusterCharacteristics(clusterResults, features, data) {
        const clusters = [];
    ...
    generateClusterCharacteristics(centroid) {
        const characteristics = [];
        if (centroid['return'] > 10)
            characteristics.push('High Return Potential');
        if (centroid['volatility'] > 15)
            characteristics.push('High Volatility');
        if (centroid['beta'] > 1.2)
            characteristics.push('High Beta');
        if (centroid['correlation'] < 0.3)
            characteristics.push('Low Correlation');
        return characteristics.length > 0 ? characteristics : ['Balanced Profile'];
    }
    calculateSilhouetteScore(featureMatrix, assignments) {
        // Mock silhouette score calculation
        return 0.6 + Math.random() * 0.3; // Score between 0.6-0.9
    }
    async detectPattern(data, patternType) {
        const patterns = [];
    ...
    async generateRebalancingSuggestion(portfolioData, objective) {
        return {
            type: 'rebalancing',
            priority: 'medium',
            expectedImpact: {
    ...
    async generateRiskReductionSuggestions(portfolioData) {
        return [
            {
                type: 'risk_reduction',
                priority: 'high',
    ...
    async generateYieldEnhancementSuggestions(portfolioData) {
        return [
            {
    ...
    async generateTaxOptimizationSuggestions(portfolioData) {
        return [
            {
    ...
    filterAndRankOptimizations(suggestions, constraints) {
        return suggestions
            .filter(suggestion => {
            // Check if suggestion violates any constraints
            return !constraints.some(constraint => suggestion.constraints.some(suggestionConstraint => suggestionConstraint.toLowerCase().includes(constraint.toLowerCase())));
        })
            .sort((a, b) => {
    ...
    async identifyPerformanceDrivers(portfolioData, benchmarkData) {
        return [
            {
    ...
    async performAttributionAnalysis(portfolioData, benchmarkData) {
        return {
    ...
    async decomposeRisk(portfolioData) {
        return {
            systematic: 78.5,
            idiosyncratic: 21.5,
            sectors: {
...
exports.MachineLearningInsightsService = MachineLearningInsightsService;
