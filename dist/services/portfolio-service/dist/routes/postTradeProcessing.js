...
const postTradeProcessingService_1 = require("../services/postTradeProcessingService");
...
const postTradeService = new postTradeProcessingService_1.PostTradeProcessingService(prisma, kafkaService);
// Trade Confirmation Routes
// Create trade confirmation
router.post('/trade-confirmations', async (req, res) => {
    try {
        const { tradeId, orderId, executionId, counterpartyId, confirmationMethod, notes } = req.body;
        // Validation
        ...
        logger_1.logger.error('Error creating trade confirmation:', error);
        res.status(500).json({
            success: false,
...
router.put('/trade-confirmations/:confirmationId', async (req, res) => {
    try {
        const { confirmationId } = req.params;
        const { confirmationStatus, affirmedAt, notes } = req.body;
        // Validate confirmation status
        const validStatuses = Object.values(PostTradeProcessing_1.TradeConfirmationStatus);
        ...
        logger_1.logger.error('Error updating trade confirmation:', error);
...
router.get('/trade-confirmations', async (req, res) => {
    try {
        const { portfolioIds, instrumentIds, counterpartyIds, confirmationStatuses, fromDate, toDate, limit, offset } = req.query;
        const searchRequest = {
            portfolioIds: portfolioIds ? portfolioIds.split(',') : undefined,
            instrumentIds: instrumentIds ? instrumentIds.split(',') : undefined,
            counterpartyIds: counterpartyIds ? counterpartyIds.split(',') : undefined,
            confirmationStatuses: confirmationStatuses ?
                confirmationStatuses.split(',') : undefined,
        ...
        logger_1.logger.error('Error searching trade confirmations:', error);
        res.status(500).json({
            success: false,
...
router.post('/settlement-instructions', async (req, res) => {
    try {
        const { tradeConfirmationId, instructionType, deliveryAccount, receiveAccount, cashAccount, priority } = req.body;
        // Validation
        ...
        const validInstructionTypes = Object.values(PostTradeProcessing_1.SettlementInstructionType);
        ...
        logger_1.logger.error('Error creating settlement instruction:', error);
        res.status(500).json({
            success: false,
...
// Create trade break
router.post('/trade-breaks', async (req, res) => {
    try {
        const { tradeId, breakType, severity, description, expectedValue, actualValue, assignedTo } = req.body;
        // Validation
        ...
        const validBreakTypes = Object.values(PostTradeProcessing_1.TradeBreakType);
        ...
        const validSeverities = Object.values(PostTradeProcessing_1.TradeBreakSeverity);
        ...
        logger_1.logger.error('Error creating trade break:', error);
        res.status(500).json({
            success: false,
...
// Resolve trade break
router.put('/trade-breaks/:breakId/resolve', async (req, res) => {
    try {
        ...
        const resolvedBreak = await postTradeService.resolveTradeBreak(breakId, resolutionNotes, req.user.tenantId, req.user.userId);
        res.json({
            success: true,
            data: resolvedBreak,
        ...
        logger_1.logger.error('Error resolving trade break:', error);
        const statusCode = error instanceof Error && error.message.includes('not found') ? 404 : 500;
        res.status(statusCode).json({
            success: false,
...
// Run TCA analysis
router.post('/transaction-cost-analysis', async (req, res) => {
    try {
        ...
        const validAnalysisTypes = Object.values(PostTradeProcessing_1.TransactionCostAnalysisType);
        ...
        logger_1.logger.error('Error running TCA analysis:', error);
        res.status(500).json({
            success: false,
...
router.get('/orders/:orderId/transaction-cost-analysis', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error fetching TCA analyses:', error);
        res.status(500).json({
            success: false,
...
router.post('/custodian-messages', async (req, res) => {
    try {
        const { custodianId, messageType, messageContent, relatedTradeId, priority } = req.body;
        // Validation
        ...
        const validMessageTypes = Object.values(PostTradeProcessing_1.CustodianMessageType);
        ...
        logger_1.logger.error('Error sending custodian message:', error);
        res.status(500).json({
            success: false,
...
router.get('/custodian-messages', async (req, res) => {
    try {
        ...
        res.json({
            ...
            pagination: {
                ...
                hasMore: (offset ? parseInt(offset) : 0) + messages.length < total
            }
        });
    }
    catch (error) {
        logger_1.logger.error('Error fetching custodian messages:', error);
        res.status(500).json({
            success: false,
...
router.post('/regulatory-reports', async (req, res) => {
    try {
        const { reportType, reportingPeriodStart, reportingPeriodEnd, regulatorId, reportData } = req.body;
        // Validation
        ...
        const validReportTypes = Object.values(PostTradeProcessing_1.RegulatoryReportType);
        ...
        logger_1.logger.error('Error creating regulatory report:', error);
        res.status(500).json({
            success: false,
...
router.get('/regulatory-reports', async (req, res) => {
    try {
        ...
        if (fromDate || toDate) {
            ...
            if (fromDate)
                where.reportingPeriodStart.gte = new Date(fromDate);
            if (toDate)
                where.reportingPeriodStart.lte = new Date(toDate);
        }
        ...
        res.json({
            ...
            pagination: {
                ...
                limit: limit ? parseInt(limit) : 50,
                offset: offset ? parseInt(offset) : 0,
                hasMore: (offset ? parseInt(offset) : 0) + reports.length < total
            }
        });
    }
    catch (error) {
        logger_1.logger.error('Error fetching regulatory reports:', error);
        res.status(500).json({
            success: false,
...
// Match trade
router.post('/trade-matching', async (req, res) => {
    try {
        const { internalTradeId, externalTradeId, priceToleranceBps, quantityToleranceShares, dateToleranceDays } = req.body;
        // Validation
        ...
        logger_1.logger.error('Error matching trade:', error);
        res.status(500).json({
            success: false,
...
router.get('/summary', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error fetching post-trade summary:', error);
        res.status(500).json({
            success: false,
...
router.get('/reference-data', async (req, res) => {
    try {
        res.json({
            success: true,
            data: {
                tradeConfirmationStatuses: Object.values(PostTradeProcessing_1.TradeConfirmationStatus),
                tradeBreakTypes: Object.values(PostTradeProcessing_1.TradeBreakType),
                tradeBreakSeverities: Object.values(PostTradeProcessing_1.TradeBreakSeverity),
                settlementInstructionTypes: Object.values(PostTradeProcessing_1.SettlementInstructionType),
                custodianMessageTypes: Object.values(PostTradeProcessing_1.CustodianMessageType),
                regulatoryReportTypes: Object.values(PostTradeProcessing_1.RegulatoryReportType),
                transactionCostAnalysisTypes: Object.values(PostTradeProcessing_1.TransactionCostAnalysisType)
            }
        });
    }
    catch (error) {
...
router.get('/health', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Post-trade processing health check failed:', error);
        res.status(503).json({
            success: false,
            status: 'unhealthy',
