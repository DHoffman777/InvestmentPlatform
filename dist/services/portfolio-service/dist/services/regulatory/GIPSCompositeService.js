...
class GIPSCompositeService {
    ...
    async createComposite(data) {
        try {
            logger_1.logger.info('Creating GIPS composite', {
                tenantId: data.tenantId,
                compositeName: data.compositeName
            });
            const compositeId = (0, crypto_1.randomUUID)();
            const composite = {
                id: compositeId,
                tenantId: data.tenantId,
                compositeName: data.compositeName,
            ...
            logger_1.logger.error('Error creating GIPS composite:', error);
            throw error;
        }
    }
    async addPerformanceData(compositeId, performanceData) {
        try {
            const composite = this.composites.get(compositeId);
            if (!composite) {
                throw new Error('GIPS composite not found');
            }
            logger_1.logger.info('Adding performance data to GIPS composite', {
                compositeId,
            ...
            for (const data of newData) {
                if (composite.performanceData.length >= 3) {
                    data.percentage3YearStandardDeviation = this.calculate3YearStandardDeviation(composite.performanceData, data.year);
                }
                if (data.numberOfPortfolios > 1) {
                    data.compositeDispersion = this.calculateCompositeDispersion(data.compositeGrossReturn, data.numberOfPortfolios);
                }
            }
            ...
            logger_1.logger.error('Error adding performance data to GIPS composite:', error);
            throw error;
        }
    }
    async validateComposite(compositeId) {
        try {
            const composite = this.composites.get(compositeId);
            if (!composite) {
                throw new Error('GIPS composite not found');
            }
            logger_1.logger.info('Validating GIPS composite', {
                compositeId,
            ...
            logger_1.logger.error('Error validating GIPS composite:', error);
            throw error;
        }
    }
    async generateGIPSReport(compositeId) {
        try {
            const composite = this.composites.get(compositeId);
            if (!composite) {
                throw new Error('GIPS composite not found');
            }
            logger_1.logger.info('Generating GIPS report', {
                compositeId,
                compositeName: composite.compositeName
            });
            ...
            logger_1.logger.error('Error generating GIPS report:', error);
            throw error;
        }
    }
    async getComposite(compositeId) {
        return this.composites.get(compositeId) || null;
    }
    async getCompositesByTenant(tenantId) {
        return Array.from(this.composites.values()).filter(composite => composite.tenantId === tenantId);
    }
    async updateComposite(compositeId, updates) {
        const composite = this.composites.get(compositeId);
        if (!composite) {
            throw new Error('GIPS composite not found');
        }
    ...
    async terminateComposite(compositeId, terminationDate, reason) {
        const composite = this.composites.get(compositeId);
        if (!composite) {
            throw new Error('GIPS composite not found');
        }
    ...
    calculate3YearStandardDeviation(performanceData, year) {
    ...
    calculateCompositeDispersion(compositeReturn, numberOfPortfolios) {
    ...
    calculateTrackingError(performanceData) {
    ...
    calculateInformationRatio(performanceData) {
    ...
    generateComplianceChecklist(composite) {
        return [
            {
...
exports.GIPSCompositeService = GIPSCompositeService;
