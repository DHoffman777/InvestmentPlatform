...
exports.RegulatoryValidatorService = void 0;
...
const ComplianceAuditService_1 = require("./services/ComplianceAuditService");
const ComplianceReportingService_1 = require("./services/ComplianceReportingService");
class RegulatoryValidatorService extends events_1.EventEmitter {
    config;
    app;
    ruleEngine;
    auditService;
    ...
    initializeServices() {
        this.ruleEngine = new RegulatoryRuleEngine_1.RegulatoryRuleEngine(this.config);
        this.auditService = new ComplianceAuditService_1.ComplianceAuditService(this.config);
        this.reportingService = new ComplianceReportingService_1.ComplianceReportingService(this.config, this.auditService, this.ruleEngine);
    }
    setupMiddleware() {
    ...
    setupRoutes() {
        ...
        this.app.post('/api/v1/validate/batch', async (req, res) => {
            try {
                const requests = req.body.requests.map((reqData) => ({
        ...
        this.app.post('/api/v1/rules', async (req, res) => {
            try {
                ...
                await this.auditService.recordAuditEvent('rule', rule.id, 'RULE_UPDATE', {
                    action: 'rule_created',
                    newValue: rule,
        ...
        this.app.put('/api/v1/rules/:ruleId', async (req, res) => {
            try {
                ...
                if (success) {
                    ...
                    await this.auditService.recordAuditEvent('rule', ruleId, 'RULE_UPDATE', {
                        action: 'rule_updated',
                        oldValue: oldRule,
        ...
        this.app.get('/api/v1/exceptions', async (req, res) => {
            try {
                ...
                if (entityType && entityId) {
                    const exceptions = await this.auditService.getEntityExceptions(entityType, entityId);
                    res.json({ exceptions });
                }
                else if (ruleId) {
                    const exceptions = await this.auditService.getRuleExceptions(ruleId);
                    res.json({ exceptions });
                }
                else {
                    res.status(400).json({
        ...
        this.app.put('/api/v1/exceptions/:exceptionId', async (req, res) => {
            try {
                ...
                const exception = await this.auditService.updateException(exceptionId, updates, updatedBy);
                if (exception) {
                    res.json(exception);
                }
                else {
        ...
        this.app.get('/api/v1/alerts', async (req, res) => {
            try {
                ...
                if (entityType && entityId) {
                    const alerts = await this.auditService.getEntityAlerts(entityType, entityId);
                    res.json({ alerts });
                }
                else {
        ...
        this.app.post('/api/v1/reports/generate', async (req, res) => {
            try {
                const { reportType, title, description, period, scope, customSections } = req.body;
                const report = await this.reportingService.generateComplianceReport(reportType, title, description, {
        ...
        this.app.get('/api/v1/audit', async (req, res) => {
            try {
                const { entityType, entityId, eventType, startDate, endDate, limit = 100 } = req.query;
                const auditTrail = await this.auditService.getAuditTrail(entityType, entityId, eventType, startDate ? new Date(startDate) : undefined, endDate ? new Date(endDate) : undefined, parseInt(limit));
                res.json({ auditTrail });
            }
            catch (error) {
                res.status(500).json({
        ...
        this.app.get('/api/v1/metrics', async (req, res) => {
            try {
                ...
                const metrics = await this.auditService.getComplianceMetrics(start, new Date(endDate));
    ...
    scheduleReports() {
        ...
        this.scheduledTasks.push(node_cron_1.default.schedule('0 8 * * *', async () => {
            try {
                const endDate = new Date();
                const startDate = new Date(endDate.getTime() - 24 * 60 * 60 * 1000);
                const report = await this.reportingService.generateComplianceReport('DAILY', 'Daily Compliance Report', 'Automated daily compliance summary', { startDate, endDate }, {
        ...
        this.scheduledTasks.push(node_cron_1.default.schedule('0 9 * * 1', async () => {
            try {
                const endDate = new Date();
                const startDate = new Date(endDate.getTime() - 7 * 24 * 60 * 60 * 1000);
                const report = await this.reportingService.generateComplianceReport('WEEKLY', 'Weekly Compliance Report', 'Automated weekly compliance analysis', { startDate, endDate }, {
                    entityTypes: ['client', 'portfolio', 'transaction'],
                    jurisdictions: ['US', 'EU'],
                    frameworks: ['SEC', 'FINRA', 'GDPR']
                });
    ...
    async validateCompliance(request) {
        ...
        try {
            ...
            // Record audit trail
            await this.auditService.recordValidationAudit(result, request.context?.userId || 'system', {
            ...
            await this.auditService.recordAuditEvent(request.entity.type, request.entity.id, 'VALIDATION', {
...
exports.RegulatoryValidatorService = RegulatoryValidatorService;
