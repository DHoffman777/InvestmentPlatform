...
class RegulatoryRuleEngine extends events_1.EventEmitter {
    config;
    rules = new Map();
    ...
    async validateEntity(request) {
        const startTime = Date.now();
        const entityData = request.entity.data;
        const rulesToValidate = request.rules
            ? request.rules.map(id => this.rules.get(id)).filter(Boolean)
    ...
    async validateRule(rule, entityData, request) {
    ...
    generateRuleMessage(rule, status, data) {
        if (status === 'PASS') {
    ...
    extractRelevantValues(conditions, data) {
        const values = {};
        for (const condition of conditions) {
    ...
    generateRecommendation(rule, result, data) {
        if (result.status !== 'FAIL') {
            return null;
        }
    ...
    getRulesByJurisdiction(jurisdiction) {
        return Array.from(this.rules.values()).filter(rule => rule.jurisdiction === jurisdiction || rule.jurisdiction === 'GLOBAL');
    }
}
