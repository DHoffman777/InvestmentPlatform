...
class CachingStrategy extends events_1.EventEmitter {
    redis;
    clusterClient;
    strategies = new Map();
    metrics = new Map();
    ...
    initializeRedis() {
        if (this.config.redis.cluster) {
            this.clusterClient = new ioredis_1.default.Cluster(this.config.redis.cluster.nodes, this.config.redis.cluster.options);
            this.redis = this.clusterClient;
        }
        else {
    ...
    updateMetrics(key, operation, data) {
    ...
    async executeWarmupStrategy(strategy) {
    ...
    generatePatternRecommendations(pattern, hitRate, averageSize, metrics) {
        ...
        const oldestMetric = metrics.reduce((oldest, current) => current.createdAt < oldest.createdAt ? current : oldest);
    ...
    generateGlobalRecommendations(metrics, memoryInfo) {
        const recommendations = [];
        // High eviction rate
