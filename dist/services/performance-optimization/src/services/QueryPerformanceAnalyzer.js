...
class QueryPerformanceAnalyzer extends runtime_1.PrismaClient {
    ...
    analyzeQuery(query) {
        const matchedPatterns = this.financialQueryPatterns.filter(pattern => pattern.sqlPattern.test(query));
    ...
    generateOptimizationPlan(query, pattern) {
        ...
        if (pattern.recommendedIndexes.length > 0) {
            optimizationSteps.push({
                ...
                sqlCommands: pattern.recommendedIndexes.map(idx => `CREATE INDEX CONCURRENTLY ${idx} ON ${this.extractTableName(query)} (${this.extractColumns(query, idx)});`),
                expectedGain: 40,
            });
            estimatedGain += 40;
        }
    ...
    extractColumns(query, indexName) {
    ...
    optimizePerformanceCalculation(query) {
    ...
    optimizeValuationQuery(query) {
        // Optimize real-time valuation queries
        let optimized = query;
        // Add hints for join optimization
        if (query.includes('JOIN market_data')) {
            optimized = optimized.replace('JOIN market_data', 'JOIN market_data /*+ USE_INDEX(idx_market_data_symbol_date) */');
        }
        return optimized;
    }
    optimizeRiskAggregation(query) {
    ...
    optimizeTransactionHistory(query) {
        // Optimize transaction history queries
        let optimized = query;
        // Add date range optimization
        if (query.includes('WHERE') && query.includes('date')) {
            optimized = optimized.replace('WHERE', 'WHERE /*+ INDEX_RANGE_SCAN(transactions idx_transactions_date_symbol) */');
        }
        return optimized;
    }
    ...
    needsSchemaOptimization(query, pattern) {
        // Determine if schema changes would benefit this query pattern
        return pattern.estimatedImpact === 'HIGH' && (query.includes('GROUP BY') ||
            query.includes('SUM(') ||
            query.includes('COUNT(') ||
            query.includes('BETWEEN'));
    }
    generateSchemaOptimizations(query, pattern) {
        const optimizations = [];
        switch (pattern.id) {
            case 'portfolio_performance_calc':
                optimizations.push(`CREATE MATERIALIZED VIEW mv_portfolio_performance AS
           SELECT portfolio_id, 
                  date_trunc('month', date) as month,
                  SUM(value) as total_value,
                  AVG(return_rate) as avg_return
           FROM positions 
           GROUP BY portfolio_id, date_trunc('month', date);`, `CREATE INDEX idx_mv_portfolio_performance ON mv_portfolio_performance (portfolio_id, month);`);
                break;
            case 'real_time_valuation':
                optimizations.push(`CREATE TABLE market_data_cache AS SELECT * FROM market_data WHERE date = CURRENT_DATE;`, `CREATE INDEX idx_market_data_cache_symbol ON market_data_cache (symbol);`);
                break;
            case 'risk_aggregation':
                optimizations.push(`CREATE TABLE risk_summary (
             portfolio_id UUID,
    ...
    assessImplementationRisk(steps) {
        ...
        if (hasSchemaChanges && hasComplexRewrites)
            return 'HIGH';
        if (hasSchemaChanges || hasComplexRewrites)
            return 'MEDIUM';
        return 'LOW';
    }
    generateTestingSuggestions(pattern) {
        const suggestions = [
        ...
        switch (pattern.id) {
            case 'portfolio_performance_calc':
                suggestions.push('Test with various date ranges and portfolio sizes', 'Validate calculation accuracy against known benchmarks');
                break;
            case 'real_time_valuation':
                suggestions.push('Test under high market volatility conditions', 'Verify cache invalidation works correctly');
                break;
            case 'risk_aggregation':
                suggestions.push('Test aggregation accuracy with edge cases', 'Validate risk calculations meet regulatory requirements');
                break;
        }
        return suggestions;
    }
    async benchmarkQuery(query, iterations = 10) {
    ...
    async generateOptimizationReport(queries) {
    ...
    calculatePriority(plan) {
        ...
        if (plan.implementationRisk === 'HIGH')
            priority *= 0.7;
        else if (plan.implementationRisk === 'MEDIUM')
            priority *= 0.85;
        // Increase priority for low-effort optimizations
        const hasLowEffort = plan.optimizationSteps.some(step => step.type === 'INDEX_CREATION' && step.expectedGain > 30);
        if (hasLowEffort)
            priority *= 1.2;
        return Math.round(priority);
    }
    getPriorityReason(plan) {
