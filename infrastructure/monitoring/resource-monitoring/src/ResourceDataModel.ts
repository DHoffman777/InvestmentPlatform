...
export interface MetricQuality {
  completeness: number; // 0-1, percentage of expected data points received
  accuracy: number; // 0-1, accuracy score based on validation
  timeliness: number; // 0-1, how fresh the data is
  consistency: number; // 0-1, consistency with other metrics
  validity: number; // 0-1, whether values are within expected ranges
...
export interface ResourceUtilization {
  overall: number; // 0-1, overall resource utilization
  cpu: number;
  memory: number;
  storage: number;
  network: number;
...
export interface ResourceTrends {
  short_term: TrendAnalysis; // Last 24 hours
  medium_term: TrendAnalysis; // Last 7 days
  long_term: TrendAnalysis; // Last 30 days
...
export interface GrowthAnalysis {
  rate: number; // Growth rate (e.g., 0.05 for 5% growth)
  acceleration: number; // Whether growth is accelerating or decelerating
  projected: Array<{
    timestamp: Date;
    value: number;
